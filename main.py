...
BOT_VERSION = "۱.۱.۰"
BUILD_TAG   = "v110-god-2026-09-25"   # 🛠 برچسب بیلد (نسخه عوض نمی‌شود) — برای اینکه ادمین مطمئن شود کد جدید روی هاست اجرا شده
...
MAIN_KB = reply_keyboard([
    ...
    ["💎 VIP", "🤖 دستیار هوش"],
    ["🏆 رتبه‌بندی", "⚙ تنظیمات"],
    ["🔥 ویژه‌های ۱.۱.۰"],          # 🆕 v1.1.0: هابِ ۳۰ فیچر گاد
    ["📣 کانال و گروه"],
])
    ...
    def _migrate(self):
        ...
        plan = {
            "profiles": {
                ...
                # 🆕 v1.1.0: انباره‌ی سبکِ «۳۰ فیچر گاد» (همه‌ی داده‌های ۱.۱.۰ در این یک ستون JSON است)
                "v110_json": "TEXT",
            },
...
def panel_game(chat_id, uid):
    ...
    _g_foot = ("🌟 امروز جمعه‌ی طلاییه — همه‌ی جوایز روزانه ×۲! 🔥" if golden_friday()
               else "🔥 ۳۰ ویژه‌ی ۱.۱.۰ از دکمه‌ی «ویژه‌های ۱.۱.۰» در دسترس است")
    api.send_message(chat_id,
                     ui_card("🎮 اتاق بازی",
                             [f"⚡ انرژی  {ui_bar(p['energy'], 100)}  {fn(p['energy'])}",
                              f"❤️ سلامت  {ui_bar(p['health'], 100)}  {fn(p['health'])}",
                              f"😊 شادی   {ui_bar(p['happiness'], 100)}  {fn(p['happiness'])}",
                              "",
...
def panel_economy(chat_id, uid):
    ...
    api.send_message(chat_id,
                     ui_card("💰 دفترچه‌ی اقتصاد",
                             [ui_kv("💳 موجودی نقد", f"{fmt_money(p['money'])}💰"),
                              ui_kv("🏦 سپرده بانکی", f"{fmt_money(p.get('bank_balance') or 0)}💰"),
                              ui_kv("💎 سکه طلا", fn(p.get('gems') or 0)),
                              "",
                              ui_kv("📊 نرخ درآمد سرور", f"×{get_setting('income_rate','1')}")],
                             "هر بخش رو باز کن و ثروتت رو بساز 💸", "💰"),
                     inline_keyboard(rows))
...
def panel_fun(chat_id, uid):
    ...
    _f_extra = []
    if god_hour_active():
        _f_extra.append("⚡🔥 ساعت گاد فعاله — حقوق کار ×۲! بدو کار کن!")
    if golden_friday():
        _f_extra.append("🌟 جمعه‌ی طلایی: جوایز ×۲! 🔥")
    api.send_message(chat_id,
                     ui_card("🎡 مرکز سرگرمی شهر",
                             ["🌙 همه بازی‌ها فرهنگی و بدون شرط‌بندی هستند",
                              "",
                              ui_kv("🧗 طبقه برج", fn(floor)),
                              ui_kv("🔮 طالع امروز", "کشیدی" if fortune_of(uid) else "نکشیدی"),
                              ui_kv("🏛 خزانه شهر", f"{fmt_money(treasury_amount())}💰"),
                              ui_kv("🎁 گنج", "فعال! بدو 🏃" if chest else "به‌زودی…"),
                              ui_kv("💪 باف باشگاه", ("فعال تا " + gym_active(uid)[11:16]) if gym_active(uid) else "—")]
                             + ([""] + _f_extra if _f_extra else [""]),
                             "🔥 ویژه‌های ۱.۱.۰ رو هم از منوی اصلی ببین!", "🎡"),
                     inline_keyboard(rows))
...
def panel_city(chat_id, uid):
    ...
    api.send_message(chat_id,
                     ui_card("🏙 اداره‌ی شهر",
                             [ui_kv("🏥 بیمه", ins),
                              ui_kv("🎓 مدرک تحصیلی", f"{fn(profile(uid).get('edu') or 0)}/۳"),
                              ui_kv("🗺 محله", district_name(profile(uid))),
                              "",
                              weather_line(),                       # 🆕 v1.0.7: آب‌وهوای شهر
                              f"{'⛓ در زندانی تا ' + jail[5:16] if jail else '🕊 آزادی!'}"],
                             "خدماتِ شهر رو از دکمه‌های زیر بگیر 👇", "🏙"),
                     inline_keyboard(rows))
...
_openrouter_invalid = False
# 📚 دانشِ کاملِ بازی — همیشه به پرامپتِ سیستمِ هوش اضافه می‌شود (v1.1.0)
GAME_KNOWLEDGE = (
    "دانشِ بازی «Life Simulator AI» (نسخه ۱.۱.۰، پیام‌رسان بله، تیم XR):\n"
    "- واحدِ پول 💰 تومان است و ارزِ دوم 💎 سکه طلا است (با پول واقعی از پنل «💎 VIP» خریداری می‌شود).\n"
    "- منوی اصلی ۲۴ دکمه دارد: 🎮 بازی، 👤 پروفایل، 💼 شغل، 🏠 خانه، 🏪 بازار، ⚔️ جنگ، 🏰 امپراتوری، 🕶 هک، "
    "👨‍👩‍👧 خانواده، 🤝 اتحاد، 🐾 پت، 🏦 بانک، 🎡 سرگرمی، 🏙 شهر، 💰 اقتصاد، 📚 مهارت‌ها، 👥 روابط، 🎯 ماموریت‌ها، "
    "🏆 رتبه‌بندی، 🔥 ویژه‌های ۱.۱.۰، 🤖 دستیار هوش، 📣 کانال و گروه، ⚙ تنظیمات، 💎 VIP.\n"
    "- کار: ۴۷ شغل در ۴ رده (ساده/حرفه‌ای/نخبه/لوکس). حقوق پایه از ۳۰۰ تا ۴۵٬۰۰۰ در هر شیفت. "
    "استخدام = مهارت + لول + مدرک تحصیلی (برای مشاغل نخبه). هر شیفت −۲۰⚡ و کول‌داون ۴۵ ثانیه. "
    "مشاغل حرفه‌ای مصاحبه‌ی ۲ سؤالی دارند. شادی زیر ۲۰ خطرِ اخراج دارد. پنجمین شیفتِ روز = پاداش ۳×\n"
    "- اقتصاد سخت است: حقوق ×۰.۳۵، مأموریت‌ها ×۰.۵، کسب‌وکار ×۰.۵، استریک ×۰.۵، کاوش ×۰.۵، برج ×۰.۵.\n"
    "- ضریبِ حقوق: نرخ سرور × رویداد جهانی × اتحاد × شهر تهران × مدرک × بازتولد × طالع × درجه افتخار × محله "
    "× تسلط منطقه × ساعت گاد (×۲) × هوای آفتابی. جمعه = جمعه‌ی طلایی (جوایز روزانه ×۲).\n"
    "- بورس: ۲۶ نماد (طلا، دلار، بیت‌کوین، نفت، تتر، زعفران، مس، الماس…)، نوسان هر ۲۰ دقیقه، کارمزد ۲٪، "
    "کف/سقف ضدتقلب. مسیر سریع: «💰 اقتصاد → 📈 سیگنال بورس».\n"
    "- بانک: سپرده، وام ۱۵٪ (۳ روزه)، سپرده بلندمدت ۸٪، پس‌انداز خودکار ۱۰٪ حقوق.\n"
    "- مسکن: ۶ سطح از کارتن‌خواب تا عمارت سلطنتی + قبوض روزانه + اجاره هر ۶ ساعت. محله‌ها: مرکز شهر(حقوق+۸٪)، "
    "مسکونی(استراحت)، صنعتی(کسب‌وکار)، دانشگاهی(XP+۱۰٪)، مالی، ساحلی.\n"
    "- جنگ/هک: ۴ ابزار حمله + ۴ دفاع، کول‌داون هک ۶۰۰ ثانیه، اختلاف لول مجاز ۱۰. غارت ارتشی ۹۰۰ ثانیه. "
    "دوئل با ELO و شرط‌بندی. باس روزانه، آرنا هفتگی، برج قهرمانان.\n"
    "- اتحاد (Guild): ساخت ۵٬۰۰۰💰، اهدا، جنگ، خزانه، مأموریت هفتگی، تسلط بر ۶ منطقه.\n"
    "- خانواده: خواستگاری، ازدواج، تا ۴ فرزند، سفر خانوادگی. پت: ۵ گونه + آموزش/استعداد/مسابقه.\n"
    "- شهر: انتخابات هفتگی شهردار (تعیین مالیات ۰/۵/۱۰٪)، زندان و وثیقه ۸۰۰💰، بیمه، اهدای خون، صدقه، "
    "سفر بین ۵ شهر، رادیو، روزنامه، آب‌وهوا، شکار گنج.\n"
    "- انگیزشی: استریک روزانه (۵۰۰→۳۰٬۰۰۰ در روز ۳۰)، فصل ۳۰ روزه، ماراتن XP روزانه (۳۰۰⭐ = +۳💎)، "
    "۲۶ دستاورد، مأموریت‌های روزانه و هفتگی، ۵ لیگ، ۳ بازتولد، دعوت با کمیسیون ۵٪.\n"
    "- 🔥 ویژه‌های ۱.۱.۰ (۳۰ فیچر): ماراتن ثروت، ماشین اسلات، اتاق فرار، دارت، نابغه، پیشگویی، درخت آرزوها، "
    "آزمایشگاه ژنتیک، نبرد برق‌آسا، پرورش اژدها، نقشه‌ی گنج، پروژه آزاد، نامه عاشقانه، بیمه حادثه، "
    "جعبه روزانه، سفر در زمان، بازار پیش‌بینی، کارگاه سازندگی شهر، صندوق امانات، سیرک هفتگی، طلسم‌ها، "
    "آکواریوم، تئاتر شهر، زنجیره پیروزی، تالار مشاهیر، بمب ساعتی، بازار شبانه، تاس سرنوشت، "
    "قطب‌نمای شانس (پاداش بعدی ×۲)، تاج هفته.\n"
    "- 🍀 سکه شانس از: جایزه روزانه، گردونه، کوییز، چت با هوش، برد آرنا، بازار سیاه.\n"
    "- 🗺️ نقشه‌ی گنج: ۵ تکه از فعالیت‌های ۱.۱.۰ جمع می‌شود → صندوقِ بزرگ.\n"
    "- 🚫 در بازی هیچ قمار/شرط‌بندی واقعی نیست؛ شهربازی و قرعه‌کشی کاملاً رایگان و فرهنگی‌اند و جوایز از "
    "خزانه‌ی شهر پرداخت می‌شود."
)

AI_DEFAULT_SYSTEM_PROMPT = (
    "تو «هوش بله» هستی — دستیارِ اختصاصی و حرفه‌ایِ بازی «Life Simulator AI» (نسخه ۱.۱.۰) روی پیام‌رسان بله، "
    "ساخته‌ی تیم XR.\n\n"
    "🔒 قوانینِ سخت:\n"
    "۱) فقط و فقط درباره‌ی همین بازی جواب بده: سیستم‌ها، پول درآوردن، لول‌آپ، شغل، بورس، هک و جنگ، "
    "خانواده و اتحاد، پت، خانه و شهر، ویژه‌های ۱.۱.۰، و استراتژیِ پیشرفت.\n"
    "۲) اگر سؤال ربطی به بازی نداشت، مؤدبانه بگو «من فقط راهنمای Life Simulator هستم 🙂» و بلافاصله یک نکته‌ی "
    "کاربردیِ مرتبط از خودِ بازی بده.\n"
    "۳) از وضعیتِ واقعیِ بازیکن (که زیرِ سؤال می‌آید) استفاده کن و دقیقاً برای «همان نقطه‌ای که هست» راهنمایی‌اش کن.\n"
    "۴) مسیر را دقیق و دکمه‌به‌دکمه بگو؛ مثلاً: «💼 شغل → 🛠 کار کردن» یا «🔥 ویژه‌ها → 🎰 ماشین اسلات».\n"
    "۵) فارسیِ روان، کوتاه (۳ تا ۶ خط)، بامزه و کاملاً عملیاتی بنویس؛ عدد و مقدار بده (مبالغ به تومان).\n"
    "۶) آخرِ هر پاسخ یک «نکته‌ی طلایی» کوتاه بنویس.\n"
    "۷) هرگز درباره‌ی قمار، شرط‌بندی یا موضوعِ خارج از بازی توصیه نده."
)
...
def ai_system_prompt(extra=None):
    ...
    # 🆕 v1.1.0: دانشِ کاملِ بازی همیشه ضمیمه می‌شود (حتی اگر ادمین پرامپتِ خودش را نوشته باشد)
    try:
        full = base + "\n\n" + GAME_KNOWLEDGE
    except Exception:
        full = base
...
def _ai_offline_answer(prompt):
    """🆕 v1.1.0 — فال‌بک آفلاینِ «کاملاً بازی‌محور» بر اساس کلیدواژه‌های سؤال
    (وقتی هیچ سرویس آنلاینی در دسترس نباشد). تمام پاسخ‌ها فقط درباره‌ی همین بازی هستند."""
    low = (prompt or "").lower()

    if any(w in low for w in ["ویژه", "1.1.0", "۱.۱.۰", "جدید", "جدیده", "فیچر", "امکانات تازه"]):
        return ("🔥 جدیدترین بخشِ بازی «🔥 ویژه‌های ۱.۱.۰» است: ۳۰ قابلیتِ تازه!\n"
                "شروعِ پیشنهادی: 🧭 قطب‌نمای شانس → بعد 🎁 جعبه روزانه → بعد 🎰 ماشین اسلات.\n"
                "برای درآمدِ غیرفعال: 🐉 پرورش اژدها و 🔐 صندوق امانات (۱۵٪ سود ۲۴ساعته).\n"
                "نکته طلایی: قطب‌نما رو قبل از بزرگ‌ترین جایزه مصرف کن! 🧭")
    if any(w in low for w in ["اژدها", "دراگون", "dragon"]):
        return ("🐉 اژدها از «🔥 ویژه‌ها → 🐉 پرورش اژدها» شروع می‌شه: اول تخم رو با ۳٬۰۰۰💰 بخر، "
                "بعد با ۸۰۰💰 غذا بده تا سطحش بره بالا. هر ۶ ساعت درآمدش (۴۰۰ × سطح) رو برداشت کن.\n"
                "نکته طلایی: اژدها در سطح‌های بالا بهترین درآمدِ غیرفعالِ بازیه! 🔥")
    if any(w in low for w in ["نقشه", "گنج", "تکه"]):
        return ("🗺️ نقشه‌ی گنج با ۵ تکه کامل می‌شه. تکه‌ها هنگام انجام فعالیت‌های ۱.۱.۰ پیدا می‌شن: "
                "اسلات، دارت، نابغه، نبرد، اژدها، آرزو، صندوق امانات، بمب…\n"
                "جایزه: ۶٬۰۰۰ تا ۱۵٬۰۰۰💰 + ۴۰۰⭐ + ۳ تا ۸💎\n"
                "نکته طلایی: بیشتر فعالیت کن = زودتر کامل می‌شه! 🗺️")
    if any(w in low for w in ["سکه شانس", "شانس", "طلسم", "lucky"]):
        return ("🍀 سکه شانس از اینجاها میاد: جایزه روزانه، گردونه، کوییز، چت با هوش، برد آرنا، بازار سیاه.\n"
                "خرجش: «🧿 طلسم‌ها» (شانس ×۱.۵ / XP ×۱.۵ / انرژی)، «🍀 فروشگاه سکه»، یا زیاد کردن شانسِ "
                "«🌟 درخت آرزوها» (هر سکه +۸٪).\n"
                "نکته طلایی: طلسمِ شانس + قطب‌نما = پاداش ×۳! 🤯")
    if any(w in low for w in ["بیمه", "خسارت", "دزدی", "حمله خوردم"]):
        return ("🛡️ «🔥 ویژه‌ها → 🛡️ بیمه‌ی حادثه»: با ۳۰۰💰 پوششِ امروز رو بخر؛ اگر امروز هک یا حمله خوردی، "
                "۷۰٪ خسارت رو پس می‌گیری.\n"
                "همچنین 🛡 استخدام گارد روزانه (۶۰۰💰) یک حمله رو کلاً دفع می‌کنه.\n"
                "نکته طلایی: هر روز صبح هم بیمه بگیر هم گارد — ارزشش خیلی بیشتر از هزینه‌شه! 🛡️")
    if any(w in low for w in ["صندوق", "امانات", "سود", "سرمایه گذاری", "پس انداز"]):
        return ("🔐 «🔥 ویژه‌ها → 🔐 صندوق امانات» پولت رو ۲۴ ساعت قفل می‌کنه و ۱۵٪ سودِ تضمینی می‌ده "
                "(۵k / ۲۰k / ۵۰k).\n"
                "در کنارش: 🏦 بانک → سپرده بلندمدت (۸٪ بعد از ۳ روز) و 📈 VC خطرپذیر (روزی ۲ بار، ریسکی).\n"
                "نکته طلایی: صندوق امانات امن‌ترین سودِ بازیه 💰")
    if any(w in low for w in ["بمب", "منفجر", "ساعتی"]):
        return ("🧨 بمب ساعتی یک بازیِ جمعیِ بامزه‌ست: بمب دستِ یکی از بازیکن‌هاست و ۱۵ دقیقه تا انفجار وقت داره. "
                "اگر دستِ تو بود سریع «💨 پاس بده» (+۲۰۰💰)؛ اگر منفجر شد ۶۰۰💰 جریمه می‌شی!\n"
                "نکته طلایی: اعلانِ بمب رو جدی بگیر 😄")
    if any(w in low for w in ["تاج", "هفته", "رتبه", "لیگ", "رتبه بندی"]):
        return ("👑 «تاجِ هفته» به ۳ نفرِ برترِ XP این هفته جایزه می‌ده (۲۰k/۱۲k/۷k 💰 + سکه). "
                "XP این هفته با «👤 پروفایل» دیده می‌شه و هر دوشنبه صفر می‌شه.\n"
                "برای صعود: 🎮 رویداد + 🎯 مأموریت + 🧠 کوییز + 📻 رادیو + 🗞 روزنامه، و جمعه‌ها ×۲!\n"
                "نکته طلایی: XPِ هفتگی رو آخرِ هفته چک کن 👑")
    if any(w in low for w in ["پول", "money", "درآمد", "ثروت", "پولدار", "میلیونر"]):
        return ("💰 مسیرِ پولدار شدن در ۱.۱.۰:\n"
                "۱) 🔥 ویژه‌ها: 🧭 قطب‌نما → 🎁 جعبه روزانه → 🎰 اسلات → 🐉 اژدها (درآمد غیرفعال)\n"
                "۲) 💼 شغل با بالاترین حقوقی که شرایطش رو داری + اضافه‌کاری روزانه\n"
                "۳) 🏢 شرکت بزن و کارمند استخدام کن (درآمدِ واقعی از اینجاست)\n"
                "۴) 📈 بورس: کف بخر، ۱۵-۲۵٪ رشد کرد بفروش\n"
                "۵) 🔐 صندوق امانات برای سودِ بدون ریسک\n"
                "نکته طلایی: اقتصاد عمداً سخته — صبور باش و روی درآمدِ غیرفعال سرمایه‌گذاری کن 🚀")
    if any(w in low for w in ["عشق", "ازدواج", "خانواده", "بچه", "رابطه"]):
        return ("💕 عشق: اول اعتبارت رو بالا ببر (کار + صدقه + 🩸 اهدای خون)، بعد «👨‍👩‍👧 خانواده → پیشنهاد ازدواج»، "
                "بعد 🏠 خانه بخر و بچه‌دار شو. «💌 نامه‌ی عاشقانه» (ویژه‌های ۱.۱.۰) رابطه با NPCها رو سریع می‌بره بالا.\n"
                "نکته طلایی: خونه‌ی بهتر = شادیِ خانواده‌ی بیشتر 🏡")
    if any(w in low for w in ["هک", "hack", "هکر"]):
        return ("🕶 هک: «🕶 هک → 🛒 فروشگاه هک» اول کیلاگر (۸۰۰💰) بخر. هر هک ۱۵⚡ و کول‌داون ۶۰۰ ثانیه. "
                "اختلاف لول با هدف باید زیر ۱۰ باشه. آنتی‌ویروس/فایروال برای دفاع بخر.\n"
                "اگر وقت نداری: «🔥 ویژه‌ها → 🕰️ سفر در زمان» همه‌ی کول‌داون‌ها رو پاک می‌کنه!\n"
                "نکته طلایی: بعد از هر هک، گاردِ روزانه استخدام کن 🛡️")
    if any(w in low for w in ["لول", "xp", "سطح", "level", "ارتقا"]):
        return ("⭐ لول‌آپ سریع در ۱.۱.۰: 🎯 مأموریت روزانه + 🎲 رویداد زندگی + 🧠 نابغه/کوییز + 📻 رادیو + "
                "🗞 روزنامه + 💌 نامه + 🧱 مشارکت در کارگاه شهر.\n"
                "جمعه‌ها همه‌چیز ×۲ و «⚡ ساعت گاد» حقوق رو ×۲ می‌کنه.\n"
                "نکته طلایی: 🧿 طلسمِ تجربه + 🧬 ژنِ نبوغ = XPِ خیلی بیشتر! ⭐")
    if any(w in low for w in ["بورس", "سهام", "سهم", "بازار", "دلار", "طلا", "بیت"]):
        return ("📊 بورس: «💰 اقتصاد → 📈 سیگنال بورس» رو ببین؛ هر دارایی با روند نزولی رو کف بخر و با ۱۵-۲۵٪ سود بفروش. "
                "سبدت رو پخش کن (طلا + دلار + یک سهم پرریسک). کارمزد ۲٪.\n"
                "جدید: «🔥 ویژه‌ها → 📈 بازار پیش‌بینی» — جهتِ یک نماد رو حدس بزن و جایزه بگیر.\n"
                "نکته طلایی: هیچ‌وقت همه‌ی پولت رو یک‌جا نذار 📈")
    if any(w in low for w in ["خونه", "خانه", "ملک", "آپارتمان", "ویلا", "محله"]):
        return ("🏠 خانه: «🏠 خانه → 🏪 خرید» از اتاق اجاره‌ای شروع کن (ارزون و اجاره می‌ده) و برس به عمارت سلطنتی. "
                "املاک هر ۶ ساعت اجاره می‌دن (🏠 خانه → 💰 دریافت اجاره).\n"
                "محله‌ها: 🎓 دانشگاهی (XP+۱۰٪) | 🏙 مرکز (حقوق+۸٪) | 🏭 صنعتی (کسب‌وکار+۱۰٪) | 💹 مالی.\n"
                "نکته طلایی: 🏠 دکور (ویژه‌ها) شادی و بونس می‌ده!")
    if any(w in low for w in ["شغل", "کار", "حقوق", "استخدام", "مصاحبه", "مدرک"]):
        return ("💼 شغل: «💼 شغل» ۴۷ شغل در ۴ رده داره. شرطِ استخدام = مهارت + لول + مدرک (برای نخبه‌ها). "
                "مشاغل حرفه‌ای مصاحبه‌ی ۲ سؤالی دارن (جواب‌ها منطقی‌اند!).\n"
                "مدرک از «🏙 شهر → 🎓 تحصیل» (دیپلم/لیسانس/فوق‌لیسانس) میاد.\n"
                "بدون شغل هم «🏗 کارگری روزمزد» و «🚕 تاکسی» هست.\n"
                "نکته طلایی: شادی زیر ۲۰ = خطرِ اخراج! 😊")
    if any(w in low for w in ["اتحاد", "کلن", "گروه", "guild"]):
        return ("🤝 اتحاد: با ۵٬۰۰۰💰 بساز یا به یکی بپیوند. اهدا کن، جنگ راه بنداز، مأموریت هفتگی بگیر و "
                "روی منطقه‌ها تسلط پیدا کن (هر منطقه بافِ دائمی می‌ده).\n"
                "سطحِ اتحاد روی حقوقِ همه‌ی اعضا اثر داره (تا +۱۰٪).\n"
                "نکته طلایی: اتحادِ فعال = رشدِ چندبرابری 🤝")
    if any(w in low for w in ["پت", "حیوان", "سگ", "گربه"]):
        return ("🐾 پت: «🐾 پت» ۵ گونه داره (تا 🐲 اژدها با سکه). غذا بده، بازی کن، آموزش بده (هر ۴ تمرین = +۱ لول) "
                "و توی مسابقه شرکت بده. «🔥 ویژه‌ها → 🐉 پرورش اژدها» نسخه‌ی پیشرفته‌ست.\n"
                "نکته طلایی: پتِ سطح‌بالا تو جنگ و دوئل کمکت می‌کنه 🐾")
    if any(w in low for w in ["زندان", "جرم", "جنایت", "وثیقه", "پلیس"]):
        return ("⛓ زندان: اگر «🏙 شهر → 🚨 جنایت» شکست خوردی ۲ ساعت می‌افتی تو زندان. "
                "راه‌های خروج: وثیقه ۸۰۰💰 یا «🔥 ویژه‌ها → 🕰️ سفر در زمان».\n"
                "شغلِ 👮 افسر پلیس برعکس: گشت بزن و مجرم‌ها رو بگیر و جایزه بگیر!\n"
                "نکته طلایی: قبل از جنایت، بیمه‌ی حادثه بگیر 🛡️")
    if any(w in low for w in ["جمعه", "گاد", "ساعت", "باف", "ضریب"]):
        return ("🌟 تقویت‌کننده‌ها: جمعه = جمعه‌ی طلایی (جوایز روزانه ×۲) · «⚡ ساعت گاد» توسط ادمین (حقوق ×۲) · "
                "🧿 طلسم‌ها · 🧬 ژن · 🧭 قطب‌نما (×۲) · 💪 باشگاه (+۱۵٪ قدرت جنگ) · 😎 محله و مدرک.\n"
                "نکته طلایی: این‌ها رو با هم جمع کن تا ضریب چندبرابر بشه! ⚡")
    if any(w in low for w in ["سلام", "چطوری", "خوبی", "hello", "hi", "درود"]):
        return ("سلام رفیق! 😎 من «هوش بله»ام — راهنمای اختصاصیِ بازی Life Simulator (نسخه ۱.۱.۰).\n"
                "هرچی درباره‌ی بازی بخوای می‌دونم: 💰 پول، ⭐ لول‌آپ، 💼 شغل، 📈 بورس، 🕶 هک، 💕 ازدواج، "
                "🔥 ویژه‌های ۱.۱.۰…\n"
                "نکته طلایی: همین الان «🔥 ویژه‌های ۱.۱.۰» رو باز کن و 🧭 قطب‌نما رو بچرخون!")
    return ("🤖 من فقط راهنمای بازی Life Simulator هستم 🙂\n"
            "نکته‌ی کاربردیِ امروز: «🔥 ویژه‌های ۱.۱.۰» رو باز کن — 🧭 قطب‌نما، 🎁 جعبه روزانه، "
            "🎰 اسلات و 🐉 اژدها سریع‌ترین راهِ رشدن.\n"
            "سؤالت رو دقیق‌تر بپرس تا دقیق‌تر راهنماییت کنم! 😉")
...
def _ai_user_context(p):
    """🆕 v1.1.0 — خلاصه‌ی کامل و واقعیِ وضعیت بازیکن برای پرامپتِ هوش
    (اسم، لول، دارایی، ویژگی‌ها، شغل، مهارت‌ها، خانه/محله، استریک، اتحاد، خانواده، پت،
     وضعیتِ ویژه‌های ۱.۱.۰) تا هوش دقیقاً برای «همین نقطه» راهنمایی کند."""
    uid = p.get("user_id")
    lines = []
    try:
        lvl = int(p.get("level") or 1)
        lines.append(f"▫️ نام: {p.get('name') or 'رفیق'} | لول {lvl} (XP {int(p.get('xp') or 0)}؛ لول بعدی با {lvl * 100} XP)")
        lines.append(f"▫️ دارایی: نقد {fmt_money(p.get('money') or 0)}💰 | بانک {fmt_money(p.get('bank_balance') or 0)}💰 | سکه {int(p.get('gems') or 0)}💎")
        lines.append(f"▫️ وضعیت: انرژی {int(p.get('energy') or 0)}⚡ | سلامت {int(p.get('health') or 0)}❤️ | شادی {int(p.get('happiness') or 0)}😊 | اعتبار {int(p.get('reputation') or 0)}🏆")
        job = "بی‌کار"
        try:
            if p.get("job_id"):
                j = db.fetchone("SELECT title FROM jobs WHERE id=?", (p.get("job_id"),))
                if j:
                    job = f"{j['title']} (سطح {int(p.get('job_level') or 1)})"
        except Exception:
            pass
        lines.append(f"▫️ شغل: {job} | مدرک تحصیلی: سطح {int(p.get('edu') or 0)}/۳ | بازتولد: {int(p.get('rebirth') or 0)}/۳")
        try:
            sk = get_skills(uid)
            top = sorted(sk.items(), key=lambda kv: kv[1], reverse=True)[:3]
            lines.append("▫️ مهارت‌های برتر: " + "، ".join(f"{SKILLS.get(k, k)} {v}" for k, v in top))
        except Exception:
            pass
        try:
            lines.append(f"▫️ خانه: {home_name(p)} | محله: {district_name(p)} | استریک: {int(p.get('streak') or 0)} روز")
        except Exception:
            pass
        try:
            g = guild_of(uid)
            if g:
                lines.append(f"▫️ اتحاد: {g['name']} (سطح {guild_level(g)})")
            else:
                lines.append("▫️ اتحاد: ندارد")
        except Exception:
            pass
        try:
            f = family_of(uid)
            if f and f.get("spouse_id"):
                lines.append(f"▫️ خانواده: متأهل با {int(f.get('children') or 0)} فرزند")
            else:
                lines.append("▫️ خانواده: مجرد")
        except Exception:
            pass
        try:
            pet = pet_of(uid)
            if pet:
                lines.append(f"▫️ پت: {pet.get('species')} سطح {int(pet.get('level') or 1)}")
        except Exception:
            pass
        # ── وضعیتِ ویژه‌های ۱.۱.۰ ──
        try:
            d = _g110(uid)
            bits = []
            bits.append(f"سکه شانس {int(p.get('lucky_coins') or 0)}🍀")
            bits.append(f"تکه نقشه {int(d.get('map_pieces', 0) or 0)}/{G110_MAP_PIECES}")
            if d.get("dna"):
                bits.append("ژن فعال: " + next((n for k, n, _c, _dd in DNA_GENES if k == d.get("dna")), "؟"))
            if int(d.get("compass_mult", 0) or 0) > 0:
                bits.append("قطب‌نما ×۲ آماده")
            ch = d.get("charm") or {}
            if ch.get("kind") and float(ch.get("until", 0) or 0) > time.time():
                bits.append(f"طلسم {ch['kind']} فعال")
            dg = d.get("dragon") or {}
            if int(dg.get("lvl", 0) or 0) > 0:
                bits.append(f"اژدها سطح {int(dg.get('lvl'))}")
            bits.append(f"زنجیره پیروزی {int(d.get('chain', 0) or 0)}")
            lines.append("▫️ ویژه‌های ۱.۱.۰: " + "، ".join(bits))
        except Exception:
            pass
    except Exception:
        pass
    if not lines:
        return f"بازیکن «{p.get('name') or 'رفیق'}»"
    lines.append("")
    lines.append("با توجه به این وضعیت، دقیق‌ترین و سریع‌ترین قدمِ بعدیِ این بازیکن را پیشنهاد بده.")
    return "\n".join(lines)
...
def panel_ai(chat_id, uid):
    ...
    story_used = int(p.get("ai_story_count") or 0) if p.get("ai_story_day") == d else 0
    lines = [
        f"سلام {p.get('name', 'رفیق')}! من «هوش بله»ام ✨",
        f"راهنمای اختصاصیِ خودِ این بازی — با توجه به لول و داراییِ واقعیِت جواب می‌دم.",
        "",
        ui_kv("💬 چتِ امروز", f"{fn(used)}/{fn(AI_CHAT_DAILY_LIMIT)} (باقی‌مانده {fn(left)})"),
        ui_kv("🎭 داستانِ امروز", f"{fn(story_used)}/{fn(AI_STORY_DAILY_LIMIT)}"),
        ui_kv("💸 هزینه", "رایگان 🎉" if AI_CHAT_COST == 0 else fmt_money(AI_CHAT_COST) + "💰"),
    ]
    kb = [
        [("💬 چت آزاد", "v109:ai"), ("🎭 داستان‌ساز", "v109:story")],
        ...
        [("🔥 ویژه‌های ۱.۱.۰ چیست؟", "v109:ai_q:v110"), ("🐉 اژدها چطوریه؟", "v109:ai_q:dragon")],
        [("🍀 سکه شانس چی بخرم؟", "v109:ai_q:lucky"), ("🛡️ چطور ضرر نکنم؟", "v109:ai_q:safe")],
        [("🔥 ویژه‌های ۱.۱.۰", "v110:hub:0"), ("🏠 منو اصلی", "menu:main")],
    ]
    api.send_message(chat_id,
                     ui_card("🤖 دستیار هوش بازی", lines, "هر سؤالی درباره‌ی بازی داری بپرس 👇", "🤖"),
                     inline_keyboard(kb))
...
def handle_v109_callback(chat_id, uid, data, cb_id, message_id):
    ...
    if cmd == "ai": ai_chat_go(chat_id, uid); api.answer_callback(cb_id, "🤖")
        ...
        qmap = {
            ...
            # 🆕 v1.1.0
            "v110": "درباره‌ی ۳۰ فیچرِ جدیدِ نسخه ۱.۱.۰ توضیح بده و بگو از کدوم شروع کنم؟",
            "dragon": "پرورش اژدها در نسخه ۱.۱.۰ چطور کار می‌کنه و چقدر درآمد داره؟",
            "lucky": "سکه شانس رو بهترین شکل چطور خرج کنم؟ طلسم بخرم یا آرزو؟",
            "safe": "چطور جلوی هک شدن، حمله و ضرر مالی رو بگیرم؟",
        }
        prompt = qmap.get(q, q)
        # چک سقف روزانه
        p = profile(uid)
...
MENU_ROUTES = {
    ...
    "🔥 ویژه‌های ۱.۱.۰": (lambda c, u: panel_god(c, u)),   # 🆕 v1.1.0: هابِ ۳۰ فیچر گاد
...
def handle_callback(cb):
    ...
    # ── 🆕 v1.1.0: هابِ «۳۰ فیچر گاد» (باید قبل از بقیه باشد) ──
    if data.startswith("v110:"):
        handle_v110_callback(chat_id, uid, data, cb_id)
        return
...
# ══════════════════════════════════════════════════════════════════
#  🆕 v1.1.0 — «۳۰ فیچر گاد» + ظاهر تازه + هوشِ کاملاً بازی‌محور
# ══════════════════════════════════════════════════════════════════
#  همه‌ی داده‌های این بخش در یک ستون JSON (v110_json روی profiles) و
#  یک جدول مشترک (v110_glob) ذخیره می‌شود → بدون مهاجرتِ سنگین.
# ══════════════════════════════════════════════════════════════════

_v110_migrated = False

def ensure_v110():
    """مهاجرت ۱.۱.۰ — جدولِ مشترک + ستون JSON روی پروفایل (خود-ترمیم هم می‌شود)"""
    global _v110_migrated
    if _v110_migrated or db is None:
        return
    try:
        db.conn.executescript("""
        CREATE TABLE IF NOT EXISTS v110_glob(
        ...
        """)
        cols = {r[1] for r in db.fetchall("PRAGMA table_info(profiles)")}
        if "v110_json" not in cols:
            db.conn.execute("ALTER TABLE profiles ADD COLUMN v110_json TEXT")
        db.conn.commit()
        _v110_migrated = True
    except Exception as _e:
        try:
            log.warning(f"v110 migrate: {_e}")
        except Exception:
            pass


# ─────────────── 🎨 ابزارهای ظاهر (v1.1.0) ───────────────
UI_SEP  = "━━━━━━━━━━━━━━"
UI_SEP2 = "──────────────"

def ui_head(title, icon="✨"):
    """تیترِ تزئینیِ یک‌خطی برای بالای هر پنل"""
    return f"{icon}━「 {title} 」━{icon}"

def ui_card(title, lines, footer=None, icon="✨"):
    """کارتِ استانداردِ پنل‌ها — ظاهر یکدست در کل ربات"""
    out = [ui_head(title, icon), UI_SEP]
    for l in (lines or []):
        if l is not None:
            out.append(str(l))
    if footer:
...
def ui_bar(cur, mx, size=10):
    try:
        pct = int(max(0.0, min(100.0, (float(cur) / float(mx)) * 100.0))) if mx else 0
    except Exception:
        pct = 0
    return bar(pct, size)
...
def ui_mmss(sec):
    ...
    m, s = divmod(rem, 60)
    if h:
        return f"{fn(h)}:{fn(m):>02}:{fn(s):>02}".replace(" ", "")
    return f"{fn(m)}:{fn(s):>02}".replace(" ", "")


# ─────────────── 🧠 انباره‌ی سبکِ v1.1.0 ───────────────
def _g110(uid) -> dict:
    p = profile(uid) or {}
    return jl(p.get("v110_json"), {}) or {}

def _gs110(uid, data: dict):
    try:
        db.execute("UPDATE profiles SET v110_json=? WHERE user_id=?", (jd(data), uid))
    except Exception:
        pass

def _gv(uid, key, default=None):
    return _g110(uid).get(key, default)

def _gset(uid, key, val):
    d = _g110(uid)
    d[key] = val
    _gs110(uid, d)

def _ginc(uid, key, n=1):
    d = _g110(uid)
    d[key] = int(d.get(key, 0) or 0) + int(n)
    _gs110(uid, d)
    return d[key]

def g_reset_daily(uid):
    """سهمیه‌های روزانه را در روز جدید صفر می‌کند"""
    d = _g110(uid)
    if d.get("_day") != today():
        for k in list(d.keys()):
            if k.startswith("u:") or k.startswith("d:"):
                d.pop(k, None)
        d["_day"] = today()
        _gs110(uid, d)
    return d

def g_left(uid, key, limit) -> int:
    d = g_reset_daily(uid)
    return max(0, int(limit) - int(d.get("u:" + key, 0) or 0))

def g_use(uid, key):
    d = _g110(uid)
    d["u:" + key] = int(d.get("u:" + key, 0) or 0) + 1
    _gs110(uid, d)

def g_once_today(uid, key):
    """یک‌بار در روز — True اگر هنوز انجام نشده"""
    d = g_reset_daily(uid)
    if d.get("d:" + key) == today():
        return False
    d["d:" + key] = today()
    _gs110(uid, d)
    return True

def g_cd_left(uid, key, sec) -> int:
    d = _g110(uid)
    try:
        left = int(float(d.get("c:" + key, 0) or 0) + sec - time.time())
    except Exception:
        left = 0
    return max(0, left)

def g_cd_touch(uid, key):
    d = _g110(uid)
    d["c:" + key] = time.time()
    _gs110(uid, d)
...
def G(key, default=None):
    """حالتِ مشترکِ سراسری"""
    try:
        r = db.fetchone("SELECT data FROM v110_glob WHERE key=?", (key,))
        if r and r["data"]:
            return jl(r["data"], default)
    except Exception:
        pass
    return default
...
def GS(key, data):
    try:
        db.execute("INSERT INTO v110_glob(key,data,updated_at) VALUES(?,?,?) "
                   "ON CONFLICT(key) DO UPDATE SET data=?, updated_at=?",
                   (key, jd(data), now_iso(), jd(data), now_iso()))
    except Exception:
        pass

def g_reward(uid, money=0, xp=0, gems=0, why="جایزه ۱.۱.۰"):
    """اعطای پاداشِ یکپارچه — اثر قطب‌نما، طلسم، دی‌ان‌ای و جمعه‌ی طلایی اینجا اعمال می‌شود"""
    d = _g110(uid)
    now = time.time()
    mult = 1.0
    notes = []
    if int(d.get("compass_mult", 0) or 0) > 0:
        mult *= 2.0
        d["compass_mult"] = 0
        notes.append("🧭 قطب‌نما ×۲")
    ch = d.get("charm") or {}
    if ch.get("kind") == "luck" and float(ch.get("until", 0) or 0) > now:
        mult *= 1.5
        notes.append("🍀 طلسم شانس ×۱.۵")
    dna = d.get("dna") or ""
    if dna == "money":
        mult *= 1.05
    if golden_friday():
        mult *= 2.0
        notes.append("🌟 جمعه طلایی ×۲")
    lines = []
    if money:
        m = int(int(money) * mult)
        change_money(uid, m, "v110", why)
        lines.append(f"💰 {'+' if m > 0 else ''}{fmt_money(m)}")
    if xp:
        x = int(int(xp) * mult * (1.5 if (ch.get("kind") == "xp" and float(ch.get("until", 0) or 0) > now) else 1))
        lines.append(f"⭐ +{fn(x)}")
        lines.extend(gain_xp(uid, x))
    if gems:
        g = int(int(gems) * mult) + (1 if dna == "gem" else 0)
        add_gems(uid, g)
        lines.append(f"💎 +{fn(g)}")
    _gs110(uid, d)
    if notes:
...
def g_map_piece(uid, chance=0.20):
    """شانسِ دریافت تکه‌ی نقشه‌ی گنج هنگام انجام فعالیت‌های ۱.۱.۰"""
    try:
        if random.random() < chance:
            n = _ginc(uid, "map_pieces", 1)
            if n >= G110_MAP_PIECES and int(_gv(uid, "map_ready", 0) or 0) == 0:
                _gset(uid, "map_ready", 1)
            return f"\n🗺️ یک تکه نقشه پیدا کردی! ({fn(min(n, G110_MAP_PIECES))}/{fn(G110_MAP_PIECES)})"
    except Exception:
        pass
    return ""
...
DRAGON_CD        = 6 * 3600
BOMB_FUSE        = 900
NIGHT_FROM, NIGHT_TO = 20, 24

AQUA_FISH = ["🐟", "🐠", "🐡", "🦈", "🐙", "🦑", "🦐", "🐋"]
CHARM_SHOP = [("luck", "🍀 طلسم شانس (پاداش ×۱.۵)", 6),
              ("xp",   "🧠 طلسم تجربه (XP ×۱.۵)",   5),
              ("energy", "⚡ طلسم انرژی (انرژی فوری)", 3)]
DNA_GENES = [("money", "💰 ژنِ ثروت",  5000, "پاداش پولیِ ۱.۱.۰ +۵٪"),
             ("xp",    "🧠 ژنِ نبوغ",  5000, "XPِ ۱.۱.۰ +۵٪"),
             ("gem",   "💎 ژنِ درخشش", 8000, "هر پاداش سکه +۱💎")]
LETTERS = [("💌 یک شعر کوتاه", 6, 5),
           ("💌 یک خاطره‌ی قدیمی", 4, 4),
           ("💌 یک عذرخواهی صمیمانه", 3, 6),
           ("💌 یک دعوت به قهوه", 5, 3),
           ("💌 یک قولِ بزرگ", 8, 2)]
WISHES = [("💰 ثروت بادآورده", "money"),
          ("⭐ دانش و تجربه", "xp"),
          ("💎 درخشش و اقبال", "gem"),
          ("❤️ سلامتی و آرامش", "health")]

V110_CATALOG = [
    ("marathon", "🏆 ماراتن ثروت",   "رتبه‌ی ثروتت رو ببین و جایزه‌ی هفتگی بگیر"),
    ("slot",     "🎰 ماشین اسلات",   "۵ چرخش رایگان هر روز — سه‌تایی بزن و بترکون"),
    ("escape",   "🧩 اتاق فرار",     "۳ اتاق، ۳ معما — فرار کن و گنج ببر"),
    ("darts",    "🎯 دارت",          "۳ پرتاب؛ امتیازت جایزه‌ت رو می‌سازه"),
    ("brain",    "🧠 نابغه",         "۵ سؤال محاسباتیِ سریع — تمرینِ مغز"),
    ("oracle",   "🔮 پیشگویی امروز", "فالِ شخصی + یک بافِ رایگانِ روزانه"),
    ("wish",     "🌟 درخت آرزوها",   "یک آرزو در روز؛ سکه شانس شانست رو بالا می‌بره"),
    ("dna",      "🧬 آزمایشگاه ژنتیک", "یک ژنِ دائمی انتخاب کن و تا ابد قوی‌تر شو"),
    ("arena_q",  "⚡ نبرد برق‌آسا",  "۳ دوئل فوری با حریف هم‌سطح"),
    ("dragon",   "🐉 پرورش اژدها",   "اژدهات رو غذا بده و هر ۶ ساعت درآمد بگیر"),
    ("map",      "🗺️ نقشه‌ی گنج",    "۵ تکه جمع کن → صندوقِ بزرگ"),
    ("gig",      "🎩 پروژه‌ی آزاد",  "یک کارِ پردرآمد قبول کن و بعد از ۱۰ دقیقه نقدش کن"),
    ("letter",   "💌 نامه‌ی عاشقانه", "برای NPC نامه بفرست و رابطه‌ات رو بترکون"),
    ("life_ins", "🛡️ بیمه‌ی حادثه",  "اگر امروز خوردی، خسارتت رو پس بگیر"),
    ("loot",     "🎁 جعبه‌ی روزانه", "هر ۲۴ ساعت یک جعبه‌ی کاملاً رایگان"),
    ("time_travel", "🕰️ سفر در زمان", "انرژی رو پُر کن و همه‌ی کول‌داون‌ها رو پاک کن"),
    ("predict",  "📈 بازار پیش‌بینی", "جهتِ یک نماد رو حدس بزن و سود بگیر"),
    ("build",    "🧱 کارگاه شهر",    "همه با هم یک بنای شهر می‌سازیم — جایزه‌ی جمعی"),
    ("locker",   "🔐 صندوق امانات",  "۲۴ ساعت قفل کن، ۱۵٪ سود بگیر"),
    ("circus",   "🎪 سیرک هفتگی",    "۳ چالش عجیبِ هفته رو کامل کن"),
    ("charm",    "🧿 طلسم‌ها",       "با سکه شانس طلسم بخر و پاداش‌ها رو چندبرابر کن"),
    ("aquarium", "🎣 آکواریوم",      "۸ ماهی کلکسیون کن و ست رو کامل کن"),
    ("theater",  "🎭 تئاتر شهر",     "روی صحنه برو و با کاریزما پول بساز"),
    ("streak_battle", "🔥 زنجیره‌ی پیروزی", "پشت‌هم ببر؛ هر برد ضریب رو بالا می‌بره"),
    ("fame",     "🏅 تالار مشاهیر",  "رکورددارانِ شهر در ۶ دسته"),
    ("bomb",     "🧨 بمب ساعتی",     "بمب رو پاس بده تا منفجر نشه!"),
    ("night",    "🌙 بازار شبانه",   "فقط شب‌ها: ۳ قلم کالا با ۳۰٪ تخفیف"),
    ("dice",     "🎲 تاس سرنوشت",    "۵ پرتابِ رایگان هر روز"),
    ("compass",  "🧭 قطب‌نمای شانس", "پاداشِ بعدیِت رو دوبرابر کن"),
    ("crown",    "👑 تاجِ هفته",     "برترین‌های XP هفته جایزه می‌گیرن"),
]

V110_ROUTES = {}   # key → تابع (در انتهای بلوک پر می‌شود)


# ─────────────── 🏆 ۱) ماراتن ثروت ───────────────
def f_marathon(chat_id, uid, arg=""):
    ensure_v110()
    p = profile(uid)
    try:
        rank = db.fetchone("SELECT COUNT(*) c FROM profiles WHERE money > ?", (p["money"] or 0,))["c"] + 1
        total = db.fetchone("SELECT COUNT(*) c FROM profiles")["c"]
    except Exception:
        rank, total = 1, 1
    if rank == 1:
        rw, tier = (25000, 300, 10), "🥇 نفر اول شهر"
    elif rank <= 10:
        rw, tier = (12000, 150, 5), "🥈 جزو ۱۰ نفر برتر"
    elif rank <= 50:
        rw, tier = (6000, 80, 2), "🥉 جزو ۵۰ نفر برتر"
    else:
        rw, tier = (2000, 30, 0), "🌱 در مسیر صعود"
    if not g_once_today(uid, "marathon"):
        ok = False
    else:
        ok = True
    lines = [ui_kv("رتبه‌ی تو", f"{fn(rank)} از {fn(total)}"),
             ui_kv("جایگاه", tier),
             "",
             ui_kv("جایزه", f"{fmt_money(rw[0])}💰 | {fn(rw[1])}⭐ | {fn(rw[2])}💎")]
    kb = [[("🎁 دریافت جایزه‌ی امروز", "v110:marathon:get")],
          [("🏅 تالار مشاهیر", "v110:fame"), ("🔥 ویژه‌ها", "v110:hub:0")]]
    if not ok:
        lines.append("")
        lines.append("⏳ جایزه‌ی امروز رو قبلاً گرفتی — فردا دوباره بیا!")
        kb = [[("🏅 تالار مشاهیر", "v110:fame"), ("🔥 ویژه‌ها", "v110:hub:0")]]
    api.send_message(chat_id, ui_card("🏆 ماراتن ثروت", lines,
                                      "هر روز ثروتمندتر شو تا رتبه‌ت بالا بره 🚀", "🏆"),
                     inline_keyboard(kb))

def f_marathon_get(chat_id, uid):
    ensure_v110()
    p = profile(uid)
    try:
        rank = db.fetchone("SELECT COUNT(*) c FROM profiles WHERE money > ?", (p["money"] or 0,))["c"] + 1
        ...
        rank = 999
    if not g_once_today(uid, "marathon"):
        api.send_message(chat_id, "⏳ جایزه‌ی امروز رو گرفتی! فردا بیا 💪")
        return
    if rank == 1:
        rw = (25000, 300, 10)
    elif rank <= 10:
        ...
        rw = (2000, 30, 0)
    lines = g_reward(uid, money=rw[0], xp=rw[1], gems=rw[2], why="ماراتن ثروت")
    lines.append(g_map_piece(uid, 0.25))
    log_action(uid, "v110_marathon", f"rank={rank}")
    api.send_message(chat_id, ui_card("🏆 جایزه‌ی ماراتن", ["🎉 جایزه‌ت واریز شد!"] + lines,
                                      f"رتبه‌ی امروزت: {fn(rank)}", "🏆"))


# ─────────────── 🎰 ۲) ماشین اسلات ───────────────
def f_slot(chat_id, uid, arg=""):
    ensure_v110()
    left = g_left(uid, "slot", SLOT_DAILY)
    if left <= 0:
        api.send_message(chat_id, ui_card("🎰 ماشین اسلات",
                                          ["⏳ چرخش‌های رایگانِ امروزت تموم شد!"],
                                          f"فردا {fn(SLOT_DAILY)} چرخشِ تازه داری", "🎰"))
        return
    g_use(uid, "slot")
    reels = [pick(SLOT_SYMBOLS) for _ in range(3)]
    uniq = len(set(reels))
    if uniq == 1:
        prize = random.randint(1500, 4000)
        head = "🎊🎊 سه‌تایی! جکپات!"
    elif uniq == 2:
        prize = random.randint(300, 900)
        head = "👏 دو تا یکی درآمد!"
    else:
        prize = random.randint(50, 200)
        head = "🙂 نه بد، نه خوب!"
    lines = [f"｜ {' '.join(reels)} ｜", "", head]
    lines += g_reward(uid, money=prize, xp=12, why="اسلات")
    lines.append(g_map_piece(uid, 0.15))
    kb = [[("🎰 چرخش بعدی", "v110:slot")], [("🔥 ویژه‌ها", "v110:hub:0")]]
    api.send_message(chat_id, ui_card("🎰 ماشین اسلات", lines,
                                      f"چرخش‌های باقی‌مانده‌ی امروز: {fn(max(0, left - 1))}", "🎰"),
                     inline_keyboard(kb))


# ─────────────── 🧩 ۳) اتاق فرار ───────────────
ESCAPE_ROOMS = [
    ("اتاق ۱ — میزِ قفل‌دار", "روی میز یک یادداشت است: «من هرچه بیشتر بگیری، بیشتر جا می‌گذارم.» کدام کلید درست است؟",
     [("🕳 چاه", True), ("🧳 کیف", False), ("📚 کتاب", False), ("🔑 کلید", False)]),
    ("اتاق ۲ — راهروی آینه", "سه آینه داری: یکی دروغ می‌گوید، یکی راست، یکی تصادفی. از کدام می‌پرسی «کدام در خروجی است؟»",
     [("🪞 آینه‌ی راست‌گو", True), ("🪞 آینه‌ی دروغ‌گو", False), ("🪞 آینه‌ی تصادفی", False), ("🚪 هیچ‌کدام، در را هل بده", False)]),
    ("اتاق ۳ — گاوصندوق", "عددی که با ضرب‌کردنش در ۳ و جمع‌کردنِ ۴ با خودش برابر می‌شود چیست؟",
     [("۲", True), ("۴", False), ("۶", False), ("۰", False)]),
]

def f_escape(chat_id, uid, arg=""):
    ensure_v110()
    st = _gv(uid, "escape", None) or {"room": 0}
    room = int(st.get("room", 0))
    if room >= len(ESCAPE_ROOMS):
        _gset(uid, "escape", {"room": 0})
        api.send_message(chat_id, ui_card("🧩 اتاق فرار",
                                          ["🏁 همه‌ی اتاق‌ها رو رد کردی!"],
                                          "فردا دوباره از نو شروع می‌شه", "🧩"))
        return
    title, q, opts = ESCAPE_ROOMS[room]
    rows = [[(lab, f"v110:escape:{i}")] for i, (lab, ok) in enumerate(opts)]
    api.send_message(chat_id, ui_card("🧩 اتاق فرار",
                                      [ui_kv("مرحله", f"{fn(room + 1)} از {fn(len(ESCAPE_ROOMS))}"),
                                       "", f"🚪 {title}", "", f"❓ {q}"],
                                      "جوابِ اشتباه یعنی برگشت به اتاق اول!", "🧩"),
                     inline_keyboard(rows))

def f_escape_pick(chat_id, uid, arg):
    ensure_v110()
    st = _gv(uid, "escape", None) or {"room": 0}
    room = int(st.get("room", 0))
    if room >= len(ESCAPE_ROOMS):
    ...
    try:
        ...
        i = -1
    _t, _q, opts = ESCAPE_ROOMS[room]
    if i < 0 or i >= len(opts):
        return
    lab, ok = opts[i]
    if ok:
        lines = ["✅ درست گفتی! در باز شد…"]
        lines += g_reward(uid, money=600 + room * 500, xp=40, why="اتاق فرار")
        _gset(uid, "escape", {"room": room + 1})
        lines.append(g_map_piece(uid, 0.35))
        if room + 1 >= len(ESCAPE_ROOMS):
            lines.append("🏆 از همه‌ی اتاق‌ها فرار کردی! +۳💎")
            add_gems(uid, 3)
            lines.append(g_reward(uid, money=3000, xp=150, why="پاداش فرار کامل"))
            try:
                channel_news(f"🧩 «{profile(uid)['name']}» از اتاق فرار شهر فرار کرد! 🚪✨")
            except Exception:
                pass
        kb = [[("🚪 اتاق بعدی", "v110:escape")], [("🔥 ویژه‌ها", "v110:hub:0")]]
    else:
        lines = ["❌ اشتباه بود! تله فعال شد و برگشتی به اتاق اول 😅"]
        lines += g_reward(uid, xp=8, why="تلاشِ اتاق فرار")
        _gset(uid, "escape", {"room": 0})
        kb = [[("🔁 دوباره از اول", "v110:escape")], [("🔥 ویژه‌ها", "v110:hub:0")]]
    log_action(uid, "v110_escape", f"room={room} ok={ok}")
    api.send_message(chat_id, ui_card("🧩 اتاق فرار", lines, "معماها هر روز ثابت‌اند — هوشِ حسابی می‌خواد!", "🧩"),
                     inline_keyboard(kb))


# ─────────────── 🎯 ۴) دارت ───────────────
def f_darts(chat_id, uid, arg=""):
    ensure_v110()
    left = g_left(uid, "darts", DARTS_DAILY)
    if left <= 0:
        api.send_message(chat_id, ui_card("🎯 دارت", ["⏳ پرتاب‌های امروزت تموم شد!"],
                                          f"فردا {fn(DARTS_DAILY)} دورِ تازه", "🎯"))
        return
    g_use(uid, "darts")
    sk = get_skills(uid).get("crea", 0)
    throws = [random.randint(1, 20 + sk * 2) for _ in range(3)]
    total = sum(throws)
    if total >= 90:
        rw, tier = (2500, 80, 2), "🎯 نشانه‌زنِ حرفه‌ای!"
    elif total >= 60:
        rw, tier = (1200, 45, 0), "👏 خوب بود!"
    elif total >= 35:
        rw, tier = (500, 20, 0), "🙂 متوسط"
    else:
        rw, tier = (150, 10, 0), "😅 دستت می‌لرزه!"
    lines = [f"🎯 {' ｜ '.join(fn(t) for t in throws)}", ui_kv("مجموع", fn(total)), "", tier]
    lines += g_reward(uid, money=rw[0], xp=rw[1], gems=rw[2], why="دارت")
    lines.append(g_map_piece(uid, 0.15))
    api.send_message(chat_id, ui_card("🎯 دارت", lines,
                                      f"دورهای باقی‌مانده: {fn(max(0, left - 1))} — خلاقیت مهارتِ این بازیه 🎨", "🎯"),
                     inline_keyboard([[("🎯 دور بعدی", "v110:darts")], [("🔥 ویژه‌ها", "v110:hub:0")]]))


# ─────────────── 🧠 ۵) نابغه (محاسبات سریع) ───────────────
def f_brain(chat_id, uid, arg=""):
    ensure_v110()
    st = _gv(uid, "brain") or {}
    if st.get("day") != today() or not st.get("qs"):
        qs = []
        for _ in range(5):
            ...
            cand = []
            while len(cand) < 3:
                d = v + random.choice([-10, -5, -3, -2, 2, 3, 5, 10, 1, -1])
                if d not in wrong and d >= 0:
                    wrong.add(d)
                    cand.append(d)
            opts = cand + [v]
            random.shuffle(opts)
            qs.append({"txt": txt, "opts": opts, "ans": v})
        st = {"day": today(), "qs": qs, "i": 0, "score": 0}
        _gset(uid, "brain", st)
    i = int(st.get("i", 0))
    if i >= len(st["qs"]):
        ...
        if score == len(st["qs"]):
            bonus = 1200
        lines = [ui_kv("امتیاز", f"{fn(score)} از {fn(len(st['qs']))}")]
        if bonus:
            lines.append("🏆 فول‌مارک! بونس ویژه:")
            lines += g_reward(uid, money=bonus, xp=120, gems=2, why="نابغه فول‌مارک")
        api.send_message(chat_id, ui_card("🧠 نابغه", lines, "فردا ۵ سؤالِ تازه 🧠", "🧠"),
                         inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))
        return
    q = st["qs"][i]
    rows = [[(fn(o), f"v110:brain:{k}")] for k, o in enumerate(q["opts"])]
    api.send_message(chat_id, ui_card("🧠 نابغه",
                                      [ui_kv("سؤال", f"{fn(i + 1)} از {fn(len(st['qs']))}"),
                                       "", f"❓ {q['txt']} = چند؟"],
                                      "۵ سؤال در روز — هر پاسخِ درست جایزه دارد", "🧠"),
                     inline_keyboard(rows))

def f_brain_pick(chat_id, uid, arg):
    ensure_v110()
    st = _gv(uid, "brain") or {}
    if not st.get("qs"):
        f_brain(chat_id, uid)
    ...
    try:
        ...
        k = -1
    if 0 <= k < len(q["opts"]) and q["opts"][k] == q["ans"]:
        st["score"] = int(st.get("score", 0)) + 1
        lines = [f"✅ آفرین! {q['txt']} = {fn(q['ans'])}"]
        lines += g_reward(uid, money=250, xp=25, why="نابغه")
    else:
        lines = [f"❌ نه! جواب درست {fn(q['ans'])} بود"]
        lines += g_reward(uid, xp=5, why="نابغه")
    st["i"] = i + 1
    _gset(uid, "brain", st)
    if st["i"] >= len(st["qs"]):
        sc = int(st.get("score", 0))
        lines.append("")
        lines.append(ui_kv("نتیجه‌ی نهایی", f"{fn(sc)} از {fn(len(st['qs']))}"))
        if sc == len(st["qs"]):
            lines += g_reward(uid, money=1200, xp=120, gems=2, why="نابغه فول‌مارک")
        lines.append(g_map_piece(uid, 0.30))
        kb = [[("🔥 ویژه‌ها", "v110:hub:0")]]
    else:
        kb = [[("➡️ سؤال بعدی", "v110:brain")], [("🔥 ویژه‌ها", "v110:hub:0")]]
    api.send_message(chat_id, ui_card("🧠 نابغه", lines, "مغزت رو تیز نگه دار! 🧠", "🧠"),
                     inline_keyboard(kb))


# ─────────────── 🔮 ۶) پیشگویی امروز ───────────────
ORACLE_LINES = [
    ("🌟 امروز ستاره‌ات در اوج است — یک فرصت مالی غیرمنتظره در راهت است.", "money"),
    ("🧠 ذهنت امروز تیزتر از همیشه است؛ هرچه یاد بگیری ماندگار می‌شود.", "xp"),
    ("💎 اقبال با توست؛ یک هدیه‌ی کوچک امروز به دستت می‌رسد.", "gem"),
    ("❤️ بدنت به استراحت نیاز دارد؛ امروز مهربان‌تر با خودت باش.", "health"),
    ("🤝 یک دوست قدیمی امروز به یادت می‌افتد — پیامی در راه است.", "happy"),
    ("⚔️ رقیبانت امروز حواسشان پرت است؛ وقتِ پیشروی است.", "money"),
]

def f_oracle(chat_id, uid, arg=""):
    ensure_v110()
    if not g_once_today(uid, "oracle"):
        api.send_message(chat_id, ui_card("🔮 پیشگویی امروز",
                                          ["🔮 پیشگوییِ امروزت رو قبلاً شنیدی…"],
                                          "فردا دوباره بیا تا آینده‌ات رو بگم", "🔮"))
        return
    txt, kind = pick(ORACLE_LINES)
    lines = [f"«{txt}»", ""]
    if kind == "money":
        lines += g_reward(uid, money=800, xp=20, why="پیشگویی")
        lines.append("✨ باف: مبلغی به حسابت نشست!")
    elif kind == "xp":
        lines += g_reward(uid, xp=60, why="پیشگویی")
        lines.append("✨ باف: تجربه‌ات بیشتر شد!")
    elif kind == "gem":
        lines += g_reward(uid, gems=1, xp=15, why="پیشگویی")
        lines.append("✨ باف: یک سکه درخشید!")
    elif kind == "health":
        db.execute("UPDATE profiles SET health=MIN(100,health+12), energy=MIN(100,energy+10) WHERE user_id=?", (uid,))
        lines.append("✨ باف: ❤️ +۱۲ و ⚡ +۱۰")
    else:
        db.execute("UPDATE profiles SET happiness=MIN(100,happiness+12) WHERE user_id=?", (uid,))
        lines.append("✨ باف: 😊 +۱۲ شادی")
    lines.append(g_map_piece(uid, 0.20))
    log_action(uid, "v110_oracle", kind)
    api.send_message(chat_id, ui_card("🔮 پیشگویی امروز", lines,
                                      "هر روز یک فال و یک بافِ رایگان", "🔮"),
                     inline_keyboard([[("🌟 درخت آرزوها", "v110:wish")], [("🔥 ویژه‌ها", "v110:hub:0")]]))


# ─────────────── 🌟 ۷) درخت آرزوها ───────────────
def f_wish(chat_id, uid, arg=""):
    ensure_v110()
    left = g_left(uid, "wish", 1)
    coins = int(profile(uid).get("lucky_coins") or 0)
    if left <= 0:
        api.send_message(chat_id, ui_card("🌟 درخت آرزوها",
                                          ["🌙 امروز آرزویت رو کردی…"],
                                          "فردا دوباره زیرِ درخت منتظرتم", "🌟"))
        return
    rows = [[(lab, f"v110:wish:{k}")] for k, (lab, kind) in enumerate(WISHES)]
    api.send_message(chat_id, ui_card("🌟 درخت آرزوها",
                                      [f"🍃 یک آرزو در روز داری.",
                                       ui_kv("سکه شانس", f"{fn(coins)} (هر سکه +۸٪ شانس)"),
                                       ui_kv("شانسِ فعلی", f"{fn(min(95, 35 + coins * 8))}٪"),
                                       "", "کدوم آرزو رو می‌کاری؟"],
                                      "آرزوی برآورده‌نشده هم یک دلداریِ کوچک دارد 🌱", "🌟"),
                     inline_keyboard(rows))

def f_wish_pick(chat_id, uid, arg):
    ensure_v110()
    if g_left(uid, "wish", 1) <= 0:
        return
    try:
        i = int(arg)
    except Exception:
        return
    if i < 0 or i >= len(WISHES):
        return
    g_use(uid, "wish")
    lab, kind = WISHES[i]
    p = profile(uid)
    coins = int(p.get("lucky_coins") or 0)
    chance = min(0.95, 0.35 + coins * 0.08)
    lines = [f"🌱 آرزو کردی: {lab}"]
    if random.random() < chance:
        if kind == "money":
            lines.append("✨ درخت لرزید و بارانِ سکه بر سرت ریخت!")
            lines += g_reward(uid, money=random.randint(2000, 6000), xp=60, gems=1, why="آرزو")
        elif kind == "xp":
            lines.append("✨ نوری از درخت به ذهنت تابید!")
            lines += g_reward(uid, xp=250, money=500, why="آرزو")
        elif kind == "gem":
            lines.append("✨ میوه‌ای درخشان روی شاخه ظاهر شد!")
            lines += g_reward(uid, gems=random.randint(2, 5), xp=50, why="آرزو")
        else:
            lines.append("✨ نسیمی خنک وزید و تنت سبک شد!")
            db.execute("UPDATE profiles SET health=MIN(100,health+25), happiness=MIN(100,happiness+15), "
                       "energy=MIN(100,energy+20) WHERE user_id=?", (uid,))
            lines += g_reward(uid, xp=40, why="آرزو")
        lines.append(g_map_piece(uid, 0.30))
        log_action(uid, "v110_wish", f"{kind}:ok")
    else:
        lines.append("🍂 آرزویت برآورده نشد… ولی درخت یک برگِ طلایی بهت داد.")
        lines += g_reward(uid, money=250, xp=15, why="دلداریِ درخت")
        log_action(uid, "v110_wish", f"{kind}:miss")
    api.send_message(chat_id, ui_card("🌟 درخت آرزوها", lines,
                                      "سکه شانس جمع کن تا شانست بره بالا 🍀", "🌟"),
                     inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))


# ─────────────── 🧬 ۸) آزمایشگاه ژنتیک ───────────────
def f_dna(chat_id, uid, arg=""):
    ensure_v110()
    d = _g110(uid)
    cur = d.get("dna") or ""
    lines = [f"🧬 آزمایشگاه آماده است. یک ژن انتخاب کن و برای همیشه قوی‌تر شو.",
             ui_kv("ژنِ فعلی", next((f"{n} — {desc}" for k, n, c, desc in DNA_GENES if k == cur), "— هنوز هیچ‌کدام"))]
    rows = [[(f"{n} — {fmt_money(c)}💰", f"v110:dna:{i}")] for i, (k, n, c, desc) in enumerate(DNA_GENES)]
    for i, (_k, _n, _c, desc) in enumerate(DNA_GENES):
        lines.append(f"▫️ {desc}")
    api.send_message(chat_id, ui_card("🧬 آزمایشگاه ژنتیک", lines,
                                      "ژن فقط یک‌بار انتخاب می‌شود — با دقت انتخاب کن!", "🧬"),
                     inline_keyboard(rows + [[("🔥 ویژه‌ها", "v110:hub:0")]]))

def f_dna_pick(chat_id, uid, arg):
    ensure_v110()
    try:
        i = int(arg)
    except Exception:
        return
    if i < 0 or i >= len(DNA_GENES):
        return
    key, name, cost, desc = DNA_GENES[i]
    p = profile(uid)
    if (p.get("money") or 0) < cost:
        api.send_message(chat_id, ui_card("🧬 آزمایشگاه ژنتیک",
                                          [f"❌ برای این ژن {fmt_money(cost)}💰 لازم داری."],
                                          f"موجودی: {fmt_money(p['money'] or 0)}💰", "🧬"))
        return
    change_money(uid, -cost, "v110", f"خرید ژن {name}")
    _gset(uid, "dna", key)
    lines = [f"🧪 تزریق انجام شد! ژنِ «{name}» فعال شد.", f"▫️ اثر: {desc}"]
    lines += g_reward(uid, xp=120, why="تزریق ژن")
    log_action(uid, "v110_dna", key)
    try:
        channel_news(f"🧬 «{profile(uid)['name']}» ژنِ «{name}» رو فعال کرد! آینده از آنِ اوست ⚡")
    except Exception:
        pass
    api.send_message(chat_id, ui_card("🧬 آزمایشگاه ژنتیک", lines, "تبریک! این ژن تا ابد با توست 🧬", "🧬"),
                     inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))


# ─────────────── ⚡ ۹) نبرد برق‌آسا / 🔥 ۲۴) زنجیره پیروزی ───────────────
def _battle_power(uid):
    p = profile(uid)
    sk = get_skills(uid)
    base = (p.get("level") or 1) * 6 + sk.get("hack", 0) * 4 + sk.get("int", 0) * 3
    try:
        g = guild_of(uid)
        if g:
            base += guild_level(g) * 2
    except Exception:
        pass
    return int(base + random.randint(0, 25))

def f_arena_q(chat_id, uid, arg=""):
    ensure_v110()
    left = g_left(uid, "arena_q", ARENA_Q_DAILY)
    if left <= 0:
        api.send_message(chat_id, ui_card("⚡ نبرد برق‌آسا", ["⏳ سه نبردِ امروزت تموم شد!"],
                                          "فردا دوباره حریف داری ⚔️", "⚡"))
        return
    g_use(uid, "arena_q")
    me = _battle_power(uid)
    foe = _battle_power(uid)
    p = profile(uid)
    if (p.get("energy") or 0) < 10:
        api.send_message(chat_id, ui_card("⚡ نبرد برق‌آسا", ["🔋 برای نبرد ۱۰ انرژی لازم داری."],
                                          "برو استراحت کن 😴", "⚡"))
        return
    db.execute("UPDATE profiles SET energy=MAX(0,energy-10) WHERE user_id=?", (uid,))
    lines = [ui_kv("قدرتِ تو", fn(me)), ui_kv("قدرتِ حریف", fn(foe)), ""]
    if me >= foe:
        lines.append("⚔️ بردی! حریف زمین‌خورد!")
        lines += g_reward(uid, money=random.randint(700, 1800), xp=70, why="نبرد برق‌آسا")
        if random.random() < 0.20:
            lines += g_reward(uid, gems=1, why="غنیمت نبرد")
        log_action(uid, "v110_arena_q", "win")
    else:
        lines.append("💥 باختی! ولی چیزی یاد گرفتی.")
        lines += g_reward(uid, xp=20, why="تجربه‌ی شکست")
        log_action(uid, "v110_arena_q", "lose")
    lines.append(g_map_piece(uid, 0.20))
    api.send_message(chat_id, ui_card("⚡ نبرد برق‌آسا", lines,
                                      f"نبردهای باقی‌مانده: {fn(max(0, left - 1))}", "⚡"),
                     inline_keyboard([[("⚔️ نبرد بعدی", "v110:arena_q")], [("🔥 ویژه‌ها", "v110:hub:0")]]))

def f_streak_battle(chat_id, uid, arg=""):
    ensure_v110()
    d = _g110(uid)
    chain = int(d.get("chain", 0) or 0)
    p = profile(uid)
    if (p.get("energy") or 0) < 12:
        api.send_message(chat_id, ui_card("🔥 زنجیره‌ی پیروزی", ["🔋 ۱۲ انرژی لازم داری."],
                                          "استراحت کن و برگرد 😴", "🔥"))
        return
    db.execute("UPDATE profiles SET energy=MAX(0,energy-12) WHERE user_id=?", (uid,))
    me = _battle_power(uid) + chain * 3
    foe = _battle_power(uid) + chain * 5
    lines = [ui_kv("زنجیره‌ی فعلی", f"{fn(chain)} 🔥"),
             ui_kv("ضریب پاداش", f"×{fn(min(5, 1 + chain))}"),
             ui_kv("قدرتِ تو / حریف", f"{fn(me)} / {fn(foe)}"), ""]
    if me >= foe:
        chain += 1
        _gset(uid, "chain", chain)
        mult = min(5, chain)
        lines.append(f"🔥 بردی! زنجیره شد {fn(chain)}")
        lines += g_reward(uid, money=600 * mult, xp=50 * mult, why="زنجیره پیروزی")
        if chain >= 5:
            lines.append("👑 زنجیره‌ی طلایی! +۲💎")
            add_gems(uid, 2)
            _gset(uid, "chain", 0)
            lines.append("(زنجیره ریست شد تا دوباره بسازی!)")
        log_action(uid, "v110_chain", f"win:{chain}")
    else:
        lines.append("💥 زنجیره پاره شد!")
        lines += g_reward(uid, xp=25, why="تلاشِ زنجیره")
        _gset(uid, "chain", 0)
        log_action(uid, "v110_chain", "lose")
    lines.append(g_map_piece(uid, 0.25))
    api.send_message(chat_id, ui_card("🔥 زنجیره‌ی پیروزی", lines,
                                      "هر چه بیشتر پشت‌هم ببری، ضریب بزرگ‌تر!", "🔥"),
                     inline_keyboard([[("⚔️ حمله‌ی بعدی", "v110:streak_battle")],
                                      [("🔥 ویژه‌ها", "v110:hub:0")]]))


# ─────────────── 🐉 ۱۰) پرورش اژدها ───────────────
DRAGON_FOOD_COST = 800

def f_dragon(chat_id, uid, arg=""):
    ensure_v110()
    d = _g110(uid)
    dg = d.get("dragon") or {}
    lvl = int(dg.get("lvl", 0) or 0)
    xp = int(dg.get("xp", 0) or 0)
    need = 4 + lvl * 2
    lines = []
    if lvl <= 0:
        lines.append("🥚 یک تخمِ اژدها روی میزت است. اول باید از تخم دربیاد!")
        rows = [[("🥚 از تخم دربیار (۳٬۰۰۰💰)", "v110:dragon:hatch")],
                [("🔥 ویژه‌ها", "v110:hub:0")]]
    else:
        cd = g_cd_left(uid, "dragon", DRAGON_CD)
        income = 400 * lvl
        lines.append(f"🐉 اژدهای تو — سطح {fn(lvl)}")
        lines.append(f"🍖 تجربه: {ui_bar(xp, need)}  {fn(xp)}/{fn(need)}")
        lines.append(ui_kv("درآمدِ آماده", f"{fmt_money(income)}💰" if cd <= 0 else f"⏳ {ui_mmss(cd)} دیگر"))
        rows = []
        rows.append([(f"🍖 غذا بده ({fmt_money(DRAGON_FOOD_COST)}💰)", "v110:dragon:feed")])
        if cd <= 0:
            rows.append([("💰 برداشت درآمد", "v110:dragon:claim")])
        rows.append([("🔥 ویژه‌ها", "v110:hub:0")])
    api.send_message(chat_id, ui_card("🐉 پرورش اژدها", lines,
                                      "هر ۶ ساعت درآمد بگیر و با غذا سطحش رو ببر", "🐉"),
                     inline_keyboard(rows))

def f_dragon_act(chat_id, uid, arg):
    ensure_v110()
    p = profile(uid)
    d = _g110(uid)
    dg = d.get("dragon") or {}
    if arg == "hatch":
        if int(dg.get("lvl", 0) or 0) > 0:
            return
        if (p.get("money") or 0) < 3000:
            api.send_message(chat_id, ui_card("🐉 پرورش اژدها",
                                              [f"❌ {fmt_money(3000)}💰 لازم داری."],
                                              f"موجودی: {fmt_money(p['money'] or 0)}💰", "🐉"))
            return
        change_money(uid, -3000, "v110", "خرید تخم اژدها")
        d["dragon"] = {"lvl": 1, "xp": 0}
        _gs110(uid, d)
        lines = ["🥚💥 تخم ترک خورد… یک اژدهای کوچولو بیرون آمد!", "🐉 اژدهات متولد شد!"]
        lines += g_reward(uid, xp=150, why="تولد اژدها")
        try:
            channel_news(f"🐉 «{profile(uid)['name']}» یک اژدها به دنیا آورد! 🔥")
        except Exception:
            pass
        api.send_message(chat_id, ui_card("🐉 پرورش اژدها", lines, "حالا غذاش بده تا بزرگ بشه 🍖", "🐉"),
                         inline_keyboard([[("🍖 غذا بده", "v110:dragon:feed")], [("🔥 ویژه‌ها", "v110:hub:0")]]))
        return
    if arg == "feed":
        lvl = int(dg.get("lvl", 0) or 0)
        if lvl <= 0:
            return
        if (profile(uid).get("money") or 0) < DRAGON_FOOD_COST:
            api.send_message(chat_id, ui_card("🐉 پرورش اژدها",
                                              [f"❌ {fmt_money(DRAGON_FOOD_COST)}💰 لازم داری."], None, "🐉"))
            return
        change_money(uid, -DRAGON_FOOD_COST, "v110", "غذای اژدها")
        xp = int(dg.get("xp", 0) or 0) + 1
        dg["xp"] = xp
        need = 4 + lvl * 2
        lines = ["🍖 اژدها با ولع خورد! (+۱ تجربه)"]
        if xp >= need:
            dg["lvl"] = lvl + 1
            dg["xp"] = 0
            lines.append(f"⬆️ اژدها به سطح {fn(lvl + 1)} رسید! درآمدش بیشتر شد 🎉")
            lines += g_reward(uid, xp=100, gems=1, why="ارتقای اژدها")
            try:
                treasury_feed(200, "مالیات اژدها")
            except Exception:
                pass
        d["dragon"] = dg
        _gs110(uid, d)
        lines.append(g_map_piece(uid, 0.20))
        api.send_message(chat_id, ui_card("🐉 پرورش اژدها", lines, "سطح بالاتر = درآمد بیشتر 🐉", "🐉"),
                         inline_keyboard([[("🐉 پنل اژدها", "v110:dragon")], [("🔥 ویژه‌ها", "v110:hub:0")]]))
        return
    if arg == "claim":
        lvl = int(dg.get("lvl", 0) or 0)
        if lvl <= 0:
            return
        if g_cd_left(uid, "dragon", DRAGON_CD) > 0:
            api.send_message(chat_id, ui_card("🐉 پرورش اژدها", ["⏳ اژدها هنوز آماده نیست!"], None, "🐉"))
            return
        g_cd_touch(uid, "dragon")
        lines = [f"🔥 اژدها شعله کشید و {fmt_money(400 * lvl)}💰 برات آورد!"]
        lines += g_reward(uid, money=400 * lvl, xp=30 * lvl, why="درآمد اژدها")
        lines.append(g_map_piece(uid, 0.20))
        log_action(uid, "v110_dragon", "claim")
        api.send_message(chat_id, ui_card("🐉 پرورش اژدها", lines, "۶ ساعت دیگر دوباره 🕐", "🐉"),
                         inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))


# ─────────────── 🗺️ ۱۱) نقشه‌ی گنج ───────────────
def f_map(chat_id, uid, arg=""):
    ensure_v110()
    n = int(_gv(uid, "map_pieces", 0) or 0)
    ready = int(_gv(uid, "map_ready", 0) or 0)
    lines = [f"🗺️ تکه‌های نقشه: {ui_bar(n, G110_MAP_PIECES)}  {fn(min(n, G110_MAP_PIECES))}/{fn(G110_MAP_PIECES)}",
             "",
             "▫️ تکه‌ها هنگام انجام فعالیت‌های ۱.۱.۰ (اسلات، دارت، نبرد، اژدها، آرزو…) پیدا می‌شن."]
    rows = []
    if ready:
        lines.append("")
        lines.append("💰 نقشه کامل شده! صندوق آماده‌ی باز شدنه!")
        rows.append([("💰 باز کردن صندوق گنج", "v110:map:open")])
    rows.append([("🔥 ویژه‌ها", "v110:hub:0")])
    api.send_message(chat_id, ui_card("🗺️ نقشه‌ی گنج", lines,
                                      f"با {fn(G110_MAP_PIECES)} تکه یک صندوقِ بزرگ باز می‌شود", "🗺️"),
                     inline_keyboard(rows))

def f_map_open(chat_id, uid, arg=""):
    ensure_v110()
    if int(_gv(uid, "map_ready", 0) or 0) == 0:
        api.send_message(chat_id, ui_card("🗺️ نقشه‌ی گنج", ["❌ نقشه هنوز کامل نشده!"], None, "🗺️"))
        return
    _gset(uid, "map_ready", 0)
    _gset(uid, "map_pieces", 0)
    lines = ["🗝️ صندوقِ گنج باز شد! نور از درزش بیرون زد…", ""]
    lines += g_reward(uid, money=random.randint(6000, 15000), xp=400, gems=random.randint(3, 8), why="گنج نقشه")
    try:
        channel_news(f"🗺️ «{profile(uid)['name']}» نقشه‌ی گنج رو کامل کرد و صندوق رو گشود! 💰✨")
    except Exception:
        pass
    log_action(uid, "v110_map", "open")
    api.send_message(chat_id, ui_card("🗺️ صندوق گنج", lines, "دوباره تکه جمع کن و تکرارش کن! 🗺️", "🗺️"),
                     inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))


# ─────────────── 🎩 ۱۲) پروژه‌ی آزاد (فریلنس) ───────────────
GIGS = [("طراحی یک لوگو", 1800), ("ترجمه‌ی یک متن", 2200),
        ("تعمیر یک سایت", 3000), ("عکاسی یک مراسم", 2600),
        ("نوشتن یک مقاله", 2000), ("دوبله‌ی یک تیزر", 3400)]

def f_gig(chat_id, uid, arg=""):
    ensure_v110()
    gig = _gv(uid, "gig") or {}
    now = time.time()
    if gig and float(gig.get("until", 0) or 0) > now:
        left = int(float(gig["until"]) - now)
        api.send_message(chat_id, ui_card("🎩 پروژه‌ی آزاد",
                                          [f"📋 پروژه‌ی فعلی: {gig['name']}",
                                           ui_kv("دستمزد", f"{fmt_money(gig['pay'])}💰"),
                                           ui_kv("زمان باقی‌مانده", ui_mmss(left))],
                                          "صبر کن تا تحویل بدی ⏳", "🎩"),
                         inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))
        return
    if gig and float(gig.get("until", 0) or 0) <= now:
        api.send_message(chat_id, ui_card("🎩 پروژه‌ی آزاد",
                                          [f"✅ پروژه‌ی «{gig['name']}» آماده‌ی تحویله!"],
                                          f"دستمزد: {fmt_money(gig['pay'])}💰", "🎩"),
                         inline_keyboard([[("💰 تحویل و دریافت دستمزد", "v110:gig:claim")],
                                          [("🔥 ویژه‌ها", "v110:hub:0")]]))
        return
    if not g_once_today(uid, "gig"):
        api.send_message(chat_id, ui_card("🎩 پروژه‌ی آزاد",
                                          ["📭 امروز پروژه‌ای برات نیومده."],
                                          "فردا دوباره سر بزن", "🎩"),
                         inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))
        return
    name, pay = pick(GIGS)
    pay = int(pay * (1 + (profile(uid).get("level") or 1) * 0.05))
    api.send_message(chat_id, ui_card("🎩 پروژه‌ی آزاد",
                                      [f"📨 یک پیشنهاد کار اومد:", "",
                                       ui_kv("پروژه", name),
                                       ui_kv("دستمزد", f"{fmt_money(pay)}💰"),
                                       ui_kv("زمان", f"{fn(GIG_WAIT // 60)} دقیقه")],
                                      "قبول کن، بعد از پایان زمان دستمزدت رو نقد کن", "🎩"),
                     inline_keyboard([[("✅ قبول می‌کنم", f"v110:gig:accept")],
                                      [("🔥 ویژه‌ها", "v110:hub:0")]]))

def f_gig_act(chat_id, uid, arg):
    ensure_v110()
    if arg == "accept":
        if not g_once_today(uid, "gig"):
            return
        name, pay = pick(GIGS)
        pay = int(pay * (1 + (profile(uid).get("level") or 1) * 0.05))
        _gset(uid, "gig", {"name": name, "pay": pay, "until": time.time() + GIG_WAIT})
        api.send_message(chat_id, ui_card("🎩 پروژه‌ی آزاد",
                                          [f"👍 پروژه‌ی «{name}» رو قبول کردی!"],
                                          f"{fn(GIG_WAIT // 60)} دقیقه‌ی دیگر برگرد و دستمزدت رو بگیر", "🎩"),
                         inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))
        return
    if arg == "claim":
        gig = _gv(uid, "gig") or {}
        if not gig or float(gig.get("until", 0) or 0) > time.time():
            api.send_message(chat_id, ui_card("🎩 پروژه‌ی آزاد", ["⏳ هنوز وقت داره!"], None, "🎩"))
            return
        _gset(uid, "gig", {})
        lines = [f"💼 پروژه‌ی «{gig['name']}» تحویل داده شد!"]
        lines += g_reward(uid, money=int(gig.get("pay", 0)), xp=90, why="پروژه آزاد")
        lines.append(g_map_piece(uid, 0.25))
        log_action(uid, "v110_gig", "claim")
        api.send_message(chat_id, ui_card("🎩 پروژه‌ی آزاد", lines, "فردا پروژه‌ی تازه 🎩", "🎩"),
                         inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))


# ─────────────── 💌 ۱۳) نامه‌ی عاشقانه ───────────────
def f_letter(chat_id, uid, arg=""):
    ensure_v110()
    left = g_left(uid, "letter", 2)
    if left <= 0:
        api.send_message(chat_id, ui_card("💌 نامه‌ی عاشقانه", ["⏳ امروز فقط ۲ نامه می‌تونی بفرستی."],
                                          "فردا دوباره 💌", "💌"))
        return
    rows = [[(f"{lab} (+{r} رابطه)", f"v110:letter:{i}")] for i, (lab, r, h) in enumerate(LETTERS)]
    api.send_message(chat_id, ui_card("💌 نامه‌ی عاشقانه",
                                      [f"✍️ {fn(left)} نامه‌ی امروزت باقی‌ست.",
                                       "یکی از آشناهای شهر منتظرِ یک کلمه‌ی قشنگه…"],
                                      "نامه رابطه‌ات رو با NPCها بهتر می‌کنه", "💌"),
                     inline_keyboard(rows + [[("🔥 ویژه‌ها", "v110:hub:0")]]))

def f_letter_pick(chat_id, uid, arg):
    ensure_v110()
    if g_left(uid, "letter", 2) <= 0:
        return
    try:
        i = int(arg)
    except Exception:
    ...
    if i < 0 or i >= len(LETTERS):
        return
    g_use(uid, "letter")
    lab, rel, hap = LETTERS[i]
    npcs = get_npcs(uid)
    if not npcs:
        api.send_message(chat_id, ui_card("💌 نامه‌ی عاشقانه", ["❌ هنوز آشنایی نداری!"], None, "💌"))
        return
    npc = pick(npcs)
    change_relation(uid, npc["npc_id"], rel)
    db.execute("UPDATE profiles SET happiness=MIN(100,happiness+?) WHERE user_id=?", (hap, uid))
    lines = [f"✉️ فرستادی: {lab}", f"▫️ گیرنده: {npc['name']}",
             f"▫️ نتیجه: 🤝 رابطه +{fn(rel)} | 😊 شادی +{fn(hap)}"]
    lines += g_reward(uid, xp=25, why="نامه عاشقانه")
    lines.append(g_map_piece(uid, 0.15))
    log_action(uid, "v110_letter", npc["npc_id"])
    api.send_message(chat_id, ui_card("💌 نامه‌ی عاشقانه", lines, "کلمات، ارزان‌ترین ثروتِ دنیا هستند 💌", "💌"),
                     inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))
...
def f_life_ins(chat_id, uid, arg=""):
    ensure_v110()
    covered = _gv(uid, "ins_day") == today()
    try:
        hits = db.fetchone("SELECT COUNT(*) c FROM war_log WHERE defender=? AND created_at LIKE ?",
                           (uid, today() + "%"))["c"]
        hits += db.fetchone("SELECT COUNT(*) c FROM crime_log WHERE target=? AND created_at LIKE ?",
                            (uid, today() + "%"))["c"]
    except Exception:
        hits = 0
    lines = [ui_kv("پوشش امروز", "✅ داری" if covered else "❌ نداری"),
             ui_kv("اتفاقات امروز", f"{fn(hits)} مورد")]
    rows = []
    if not covered:
        rows.append([(f"🛡️ خرید پوشش ({fmt_money(INS_COST)}💰)", "v110:life_ins:buy")])
    elif hits > 0 and int(_gv(uid, "ins_claim", 0) or 0) != 1:
        rows.append([("💰 دریافت خسارت (۷۰٪)", "v110:life_ins:claim")])
        lines.append("")
        lines.append("💸 امروز خوردی! می‌تونی خسارت بگیری.")
    else:
        lines.append("")
        lines.append("😌 امروز در امان بودی.")
    rows.append([("🔥 ویژه‌ها", "v110:hub:0")])
    api.send_message(chat_id, ui_card("🛡️ بیمه‌ی حادثه", lines,
                                      "اگر امروز هک/حمله/دزدی خوردی، خسارتت رو پس بگیر", "🛡️"),
                     inline_keyboard(rows))

def f_life_ins_act(chat_id, uid, arg):
    ensure_v110()
    p = profile(uid)
    if arg == "buy":
        if _gv(uid, "ins_day") == today():
            return
        if (p.get("money") or 0) < INS_COST:
            api.send_message(chat_id, ui_card("🛡️ بیمه‌ی حادثه",
                                              [f"❌ {fmt_money(INS_COST)}💰 لازم داری."], None, "🛡️"))
            return
        change_money(uid, -INS_COST, "v110", "حق بیمه حادثه")
        _gset(uid, "ins_day", today())
        _gset(uid, "ins_claim", 0)
        api.send_message(chat_id, ui_card("🛡️ بیمه‌ی حادثه",
                                          ["✅ پوششِ امروز فعال شد!"],
                                          "اگر امروز ضربه بخوری، ۷۰٪ خسارت رو می‌گیری", "🛡️"),
                         inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))
        return
    if arg == "claim":
        if _gv(uid, "ins_day") != today():
            api.send_message(chat_id, ui_card("🛡️ بیمه‌ی حادثه", ["❌ امروز بیمه نداری!"], None, "🛡️"))
            return
        if int(_gv(uid, "ins_claim", 0) or 0) == 1:
            api.send_message(chat_id, ui_card("🛡️ بیمه‌ی حادثه", ["⏳ خسارتِ امروز رو گرفتی!"], None, "🛡️"))
            return
        try:
            hits = db.fetchone("SELECT COUNT(*) c FROM war_log WHERE defender=? AND created_at LIKE ?",
                               (uid, today() + "%"))["c"]
            ...
            hits = 0
        if hits <= 0:
            api.send_message(chat_id, ui_card("🛡️ بیمه‌ی حادثه", ["😌 امروز اتفاقی برات نیفتاده!"], None, "🛡️"))
            return
        _gset(uid, "ins_claim", 1)
        comp = 1500 + 900 * min(hits, 5)
        lines = [f"🏦 بیمه خسارتِ {fn(hits)} اتفاقِ امروز رو پرداخت کرد."]
        lines += g_reward(uid, money=comp, xp=40, why="خسارت بیمه")
        log_action(uid, "v110_ins", f"hits={hits}")
        api.send_message(chat_id, ui_card("🛡️ بیمه‌ی حادثه", lines, "فردا دوباره بیمه شو!", "🛡️"),
                         inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))


# ─────────────── 🎁 ۱۵) جعبه‌ی روزانه ───────────────
def f_loot(chat_id, uid, arg=""):
    ensure_v110()
    if g_cd_left(uid, "loot", LOOT_HOURS * 3600) > 0:
        api.send_message(chat_id, ui_card("🎁 جعبه‌ی روزانه",
                                          ["📦 جعبه‌ی بعدی هنوز نرسیده…"],
                                          f"{ui_mmss(g_cd_left(uid, 'loot', LOOT_HOURS * 3600))} دیگر", "🎁"))
        return
    g_cd_touch(uid, "loot")
    roll = random.random()
    lines = ["📦 جعبه رو باز کردی…"]
    if roll < 0.06:
        lines.append("💎💎 جعبه‌ی الماس! شانس با تو بود!")
        lines += g_reward(uid, money=8000, xp=300, gems=5, why="جعبه روزانه")
    elif roll < 0.30:
        lines.append("💰 دسته‌گل! پول نقد خوبی توش بود.")
        lines += g_reward(uid, money=random.randint(1500, 3500), xp=120, why="جعبه روزانه")
    elif roll < 0.60:
        lines.append("⭐ یک بسته‌ی تجربه.")
        lines += g_reward(uid, xp=random.randint(80, 200), money=400, why="جعبه روزانه")
    elif roll < 0.85:
        lines.append("🍀 چند سکه شانس!")
        _ginc(uid, "lucky_coins_bonus", 2)
        db.execute("UPDATE profiles SET lucky_coins=COALESCE(lucky_coins,0)+2 WHERE user_id=?", (uid,))
        lines += g_reward(uid, money=300, xp=30, why="جعبه روزانه")
    else:
        lines.append("❤️ یک بسته‌ی سلامتی و انرژی.")
        db.execute("UPDATE profiles SET health=MIN(100,health+20), energy=MIN(100,energy+25), "
                   "happiness=MIN(100,happiness+10) WHERE user_id=?", (uid,))
        lines += g_reward(uid, xp=30, why="جعبه روزانه")
    lines.append(g_map_piece(uid, 0.25))
    log_action(uid, "v110_loot", "")
    api.send_message(chat_id, ui_card("🎁 جعبه‌ی روزانه", lines, "هر ۲۴ ساعت یک جعبه‌ی رایگان 🎁", "🎁"),
                     inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))


# ─────────────── 🕰️ ۱۶) سفر در زمان ───────────────
def f_time_travel(chat_id, uid, arg=""):
    ensure_v110()
    if not g_once_today(uid, "time_travel"):
        api.send_message(chat_id, ui_card("🕰️ سفر در زمان",
                                          ["⌛ دستگاهِ زمان امروز خالی شده!"],
                                          "فردا می‌تونی دوباره زمان رو عقب بکشی", "🕰️"))
        return
    db.execute("UPDATE profiles SET energy=100, last_work=NULL, last_rest=NULL WHERE user_id=?", (uid,))
    try:
        db.execute("UPDATE resources SET last_hack=NULL, last_raid=NULL WHERE user_id=?", (uid,))
    except Exception:
        pass
    lines = ["🌀 زمان عقب رفت!",
             "▫️ ⚡ انرژی کاملاً پُر شد (۱۰۰)",
             "▫️ 💼 کول‌داونِ کار پاک شد",
             "▫️ 🕶 کول‌داونِ هک پاک شد",
             "▫️ 🏰 کول‌داونِ حمله پاک شد",
             "▫️ 😴 کول‌داونِ استراحت پاک شد"]
    lines += g_reward(uid, xp=60, why="سفر در زمان")
    log_action(uid, "v110_time", "")
    api.send_message(chat_id, ui_card("🕰️ سفر در زمان", lines,
                                      "یک‌بار در روز — وقتی عجله داری!", "🕰️"),
                     inline_keyboard([[("⚡ برو سر کار", "v110:hub:0")]]))


# ─────────────── 📈 ۱۷) بازار پیش‌بینی ───────────────
def f_predict(chat_id, uid, arg=""):
    ensure_v110()
    rnd = G("predict", None) or {}
    if not rnd or rnd.get("day") != today():
        rows = [dict(r) for r in db.fetchall("SELECT * FROM markets ORDER BY RANDOM() LIMIT 1")]
        if not rows:
            api.send_message(chat_id, ui_card("📈 بازار پیش‌بینی", ["❌ بازار در دسترس نیست."], None, "📈"))
            return
        rnd = {"day": today(), "sym": rows[0]["symbol"], "name": rows[0]["name"],
               "start": int(rows[0]["price"])}
        GS("predict", rnd)
    my = _gv(uid, "pred") or {}
    lines = [ui_kv("دارایی امروز", rnd["name"]),
             ui_kv("قیمتِ شروع", f"{fmt_money(rnd['start'])}💰"),
             ui_kv("حدسِ تو", "—" if my.get("day") != today() else ("⬆️ صعود" if my.get("dir") == "up" else "⬇️ سقوط"))]
    rows = []
    if my.get("day") != today():
        rows.append([("⬆️ حدس می‌زنم بالا می‌ره", "v110:predict:up"),
                     ("⬇️ حدس می‌زنم پایین می‌آد", "v110:predict:down")])
    else:
        rows.append([("📊 ببین چی شد", "v110:predict:result")])
    rows.append([("🔥 ویژه‌ها", "v110:hub:0")])
    api.send_message(chat_id, ui_card("📈 بازار پیش‌بینی", lines,
                                      "بعد از یک دورهِ بازار نتیجه رو ببین و جایزه بگیر", "📈"),
                     inline_keyboard(rows))

def _predict_round():
    """دورِ امروزِ بازار پیش‌بینی — اگر وجود ندارد ساخته‌اش کن"""
    rnd = G("predict", None) or {}
    if not rnd or rnd.get("day") != today():
        try:
            rows = [dict(r) for r in db.fetchall("SELECT * FROM markets ORDER BY RANDOM() LIMIT 1")]
...
def f_predict_act(chat_id, uid, arg):
    ensure_v110()
    rnd = _predict_round()
    if not rnd:
        api.send_message(chat_id, ui_card("📈 بازار پیش‌بینی",
                                          ["❌ بازار در دسترس نیست؛ کمی بعد دوباره تلاش کن."], None, "📈"))
        return
    if arg in ("up", "down"):
        if (_gv(uid, "pred") or {}).get("day") == today():
            api.send_message(chat_id, ui_card("📈 بازار پیش‌بینی", ["⏳ امروز حدست رو زدی!"], None, "📈"))
            return
        _gset(uid, "pred", {"day": today(), "dir": arg, "start": rnd.get("start", 0)})
        api.send_message(chat_id, ui_card("📈 بازار پیش‌بینی",
                                          [f"📝 ثبت شد: {'⬆️ صعود' if arg == 'up' else '⬇️ سقوط'}"],
                                          "بعد از یک دورهِ بازار برگرد و نتیجه رو ببین", "📈"),
                         inline_keyboard([[("📊 نتیجه", "v110:predict:result")],
                                          [("🔥 ویژه‌ها", "v110:hub:0")]]))
        return
    if arg == "result":
        my = _gv(uid, "pred") or {}
        if my.get("day") != today():
            api.send_message(chat_id, ui_card("📈 بازار پیش‌بینی", ["❌ امروز حدسی نزده بودی!"], None, "📈"))
            return
        row = db.fetchone("SELECT price FROM markets WHERE symbol=?", (rnd.get("sym"),))
        if not row:
            return
        cur = int(row["price"])
        start = int(my.get("start") or rnd.get("start") or cur)
        actual = "up" if cur >= start else "down"
        _gset(uid, "pred", {"day": today() + "_done", "dir": my.get("dir"), "start": start})
        lines = [ui_kv("قیمت شروع", f"{fmt_money(start)}💰"),
                 ui_kv("قیمت الان", f"{fmt_money(cur)}💰"),
                 ui_kv("اتفاق افتاد", "⬆️ صعود" if actual == "up" else "⬇️ سقوط"), ""]
        if my.get("dir") == actual:
            lines.append("🎯 درست پیش‌بینی کردی! تحلیل‌گرِ شهر!")
            lines += g_reward(uid, money=2200, xp=150, gems=1, why="پیش‌بینی درست")
        else:
            lines.append("📉 این بار اشتباه بود؛ ولی تجربه گرفتی.")
            lines += g_reward(uid, money=400, xp=40, why="پیش‌بینی")
        lines.append(g_map_piece(uid, 0.20))
        log_action(uid, "v110_predict", actual)
        api.send_message(chat_id, ui_card("📈 بازار پیش‌بینی", lines, "فردا یک داراییِ تازه 📈", "📈"),
                         inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))


# ─────────────── 🧱 ۱۸) کارگاه سازندگی شهر ───────────────
def f_build(chat_id, uid, arg=""):
    ensure_v110()
    st = G("build", None) or {"total": 0, "goal": BUILD_GOAL, "round": 1, "helpers": {}}
    total = int(st.get("total", 0))
    goal = int(st.get("goal", BUILD_GOAL))
    done = total >= goal
    lines = [f"🏗 بنای دوره‌ی {fn(st.get('round', 1))}: «برجِ شهر»",
             f"🧱 {ui_bar(total, goal)}  {fn(total)}/{fn(goal)}",
             ui_kv("مشارکتِ تو امروز", f"{fn(int((st.get('helpers') or {}).get(str(uid), 0)))} واحد"),
             ui_kv("سهمیه‌ی روزانه", f"{fn(BUILD_PER_DAY)} واحد")]
    rows = []
    if done:
        lines.append("")
        lines.append("🎉 ساختمان کامل شد! جایزه‌ات آماده‌ست.")
        if int(_gv(uid, "build_claim", 0) or 0) != int(st.get("round", 1)):
            rows.append([("🎁 دریافت جایزه‌ی جمعی", "v110:build:claim")])
    else:
        rows.append([("🧱 مشارکت (+۱ واحد)", "v110:build:add")])
    rows.append([("🔥 ویژه‌ها", "v110:hub:0")])
    api.send_message(chat_id, ui_card("🧱 کارگاه سازندگی شهر", lines,
                                      "همه با هم — هر کس بیشتر کمک کنه، سهمش بیشتره", "🧱"),
                     inline_keyboard(rows))

def f_build_act(chat_id, uid, arg):
    ensure_v110()
    st = G("build", None) or {"total": 0, "goal": BUILD_GOAL, "round": 1, "helpers": {}}
    helpers = st.get("helpers") or {}
    if arg == "add":
        if g_left(uid, "build", BUILD_PER_DAY) <= 0:
            api.send_message(chat_id, ui_card("🧱 کارگاه شهر", ["⏳ سهمیه‌ی امروزت تموم شد!"], None, "🧱"))
            return
        g_use(uid, "build")
        st["total"] = int(st.get("total", 0)) + 1
        helpers[str(uid)] = int(helpers.get(str(uid), 0) or 0) + 1
        st["helpers"] = helpers
        GS("build", st)
        lines = ["🧱 یک واحد مصالح گذاشتی! از طرفِ شهر ممنونیم 🙏"]
        lines += g_reward(uid, xp=45, money=250, why="مشارکت در سازندگی")
        lines.append(g_map_piece(uid, 0.15))
        if int(st["total"]) >= int(st.get("goal", BUILD_GOAL)):
            lines.append("")
            lines.append("🎉🎉 بنای شهر کامل شد! همه می‌تونن جایزه بگیرن!")
            try:
                channel_news(f"🏗 دوره‌ی {fn(st.get('round', 1))} ساختِ «برج شهر» با کمک بازیکن‌ها کامل شد! 🎉")
            except Exception:
                pass
        log_action(uid, "v110_build", "add")
        api.send_message(chat_id, ui_card("🧱 کارگاه شهر", lines,
                                          f"پیشرفت: {fn(st['total'])}/{fn(st.get('goal', BUILD_GOAL))}", "🧱"),
                         inline_keyboard([[("🧱 پنل کارگاه", "v110:build")], [("🔥 ویژه‌ها", "v110:hub:0")]]))
        return
    if arg == "claim":
        if int(st.get("total", 0)) < int(st.get("goal", BUILD_GOAL)):
            return
        if int(_gv(uid, "build_claim", 0) or 0) == int(st.get("round", 1)):
            api.send_message(chat_id, ui_card("🧱 کارگاه شهر", ["⏳ جایزه‌ات رو گرفتی!"], None, "🧱"))
            return
        share = int(helpers.get(str(uid), 0) or 0)
        _gset(uid, "build_claim", int(st.get("round", 1)))
        lines = [f"🎁 سهمِ تو از جشنِ افتتاحیه ({fn(share)} واحد مشارکت)"]
        lines += g_reward(uid, money=1500 + 600 * min(share, 10), xp=200, gems=2, why="جایزه سازندگی")
        lines.append(g_map_piece(uid, 0.25))
        api.send_message(chat_id, ui_card("🧱 کارگاه شهر", lines, "دوره‌ی بعدی به‌زودی 🧱", "🧱"),
                         inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))


# ─────────────── 🔐 ۱۹) صندوق امانات ───────────────
def f_locker(chat_id, uid, arg=""):
    ensure_v110()
    lk = _gv(uid, "locker") or {}
    if lk and float(lk.get("until", 0) or 0) > 0:
        if time.time() < float(lk["until"]):
            lines = [ui_kv("مبلغ قفل‌شده", f"{fmt_money(lk['amt'])}💰"),
                     ui_kv("سود", f"{fmt_money(int(int(lk['amt']) * LOCKER_RATE))}💰 ({fn(int(LOCKER_RATE * 100))}٪)"),
                     ui_kv("آزاد می‌شود", ui_mmss(float(lk["until"]) - time.time()))]
            kb = [[("🔥 ویژه‌ها", "v110:hub:0")]]
        else:
            amt = int(lk["amt"])
            profit = int(amt * LOCKER_RATE)
            _gset(uid, "locker", {})
            lines = ["🔓 صندوق باز شد!"]
            lines += g_reward(uid, money=amt + profit, xp=80, why="بازگشت صندوق امانات")
            lines.append(g_map_piece(uid, 0.25))
            log_action(uid, "v110_locker", "claim")
            kb = [[("🔥 ویژه‌ها", "v110:hub:0")]]
        api.send_message(chat_id, ui_card("🔐 صندوق امانات", lines,
                                          f"{fn(LOCKER_HOURS)} ساعت قفل، {fn(int(LOCKER_RATE * 100))}٪ سود", "🔐"),
                         inline_keyboard(kb))
        return
    rows = [[(f"🔐 {fmt_money(v)}💰", f"v110:locker:{v}")] for v in (5000, 20000, 50000)]
    api.send_message(chat_id, ui_card("🔐 صندوق امانات",
                                      [f"🏦 پولت رو {fn(LOCKER_HOURS)} ساعت قفل کن و {fn(int(LOCKER_RATE * 100))}٪ سودِ تضمینی بگیر.",
                                       "",
                                       "مبلغ رو انتخاب کن:"],
                                      "تا زمانِ آزاد شدن نمی‌تونی برداشتش کنی!", "🔐"),
                     inline_keyboard(rows + [[("🔥 ویژه‌ها", "v110:hub:0")]]))

def f_locker_put(chat_id, uid, arg):
    ensure_v110()
    try:
        amt = int(arg)
    except Exception:
    ...
    if (p.get("money") or 0) < amt:
        api.send_message(chat_id, ui_card("🔐 صندوق امانات",
                                          [f"❌ {fmt_money(amt)}💰 لازم داری."],
                                          f"موجودی: {fmt_money(p['money'] or 0)}💰", "🔐"))
        return
    change_money(uid, -amt, "v110", "سپرده صندوق امانات")
    _gset(uid, "locker", {"amt": amt, "until": time.time() + LOCKER_HOURS * 3600})
    log_action(uid, "v110_locker", f"put:{amt}")
    api.send_message(chat_id, ui_card("🔐 صندوق امانات",
                                      [f"🔒 {fmt_money(amt)}💰 قفل شد!",
                                       f"💰 بعد از {fn(LOCKER_HOURS)} ساعت: {fmt_money(amt + int(amt * LOCKER_RATE))}💰"],
                                      "سر وقت برگرد و برداشت کن!", "🔐"),
                     inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))


# ─────────────── 🎪 ۲۰) سیرک هفتگی ───────────────
CIRCUS = [("🎪 ۱۰ شیفت کاری در هفته", "job_work", 10, 4000, 3),
          ("🎪 ۳ نبرد موفق در هفته", "war_win", 3, 5000, 2),
          ("🎪 ۵ کاوش در هفته", "explore", 5, 4500, 2)]

def f_circus(chat_id, uid, arg=""):
    ensure_v110()
    lines = [f"🎪 سیرکِ هفته ({week_key()})", ""]
    rows = []
    for i, (title, action, need, money, gems) in enumerate(CIRCUS):
        try:
            cnt = logs_since(uid, action, week_start())
        except Exception:
            cnt = 0
        claimed = bool(int(_gv(uid, "circus" + str(i), 0) or 0) == 1)
        state = "✅ گرفته شده" if claimed else ("🎁 آماده!" if cnt >= need else f"{fn(min(cnt, need))}/{fn(need)}")
        lines.append(f"{title} — {ui_bar(min(cnt, need), need)} {state}")
        if cnt >= need and not claimed:
            rows.append([(f"🎁 جایزه‌ی چالش {fn(i + 1)}", f"v110:circus:{i}")])
    rows.append([("🔥 ویژه‌ها", "v110:hub:0")])
    api.send_message(chat_id, ui_card("🎪 سیرک هفتگی", lines,
                                      "هر هفته سه چالشِ تازه — از دوشنبه شروع می‌شه", "🎪"),
                     inline_keyboard(rows))

def f_circus_claim(chat_id, uid, arg):
    ensure_v110()
    try:
        i = int(arg)
    except Exception:
        return
    if i < 0 or i >= len(CIRCUS):
        return
    title, action, need, money, gems = CIRCUS[i]
    if int(_gv(uid, "circus" + str(i), 0) or 0) == 1:
        api.send_message(chat_id, ui_card("🎪 سیرک هفتگی", ["⏳ این جایزه رو گرفتی!"], None, "🎪"))
        return
    try:
        cnt = logs_since(uid, action, week_start())
    except Exception:
        cnt = 0
    if cnt < need:
        api.send_message(chat_id, ui_card("🎪 سیرک هفتگی",
                                          [f"🔒 هنوز کامل نشده ({fn(cnt)}/{fn(need)})"], None, "🎪"))
        return
    _gset(uid, "circus" + str(i), 1)
    lines = [f"🎪 چالشِ «{title}» رو کامل کردی!"]
    lines += g_reward(uid, money=money, gems=gems, xp=150, why="سیرک هفتگی")
    lines.append(g_map_piece(uid, 0.25))
    log_action(uid, "v110_circus", str(i))
    api.send_message(chat_id, ui_card("🎪 سیرک هفتگی", lines, "هفته‌ی بعد چالش‌های تازه 🎪", "🎪"),
                     inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))


# ─────────────── 🧿 ۲۱) طلسم‌ها ───────────────
def f_charm(chat_id, uid, arg=""):
    ensure_v110()
    p = profile(uid)
    coins = int(p.get("lucky_coins") or 0)
    d = _g110(uid)
    ...
    active = ""
    if ch.get("kind") and float(ch.get("until", 0) or 0) > time.time():
        active = f"✅ {ch['kind']} — {ui_mmss(float(ch['until']) - time.time())} باقی‌مانده"
    lines = [ui_kv("سکه شانس", fn(coins)),
             ui_kv("طلسمِ فعال", active or "— نداری"),
             "",
             "▫️ طلسمِ شانس: پاداش‌های ۱.۱.۰ ×۱.۵",
             "▫️ طلسمِ تجربه: XPِ ۱.۱.۰ ×۱.۵",
             "▫️ طلسمِ انرژی: انرژی فوری پُر می‌شود"]
    rows = [[(f"{n} — {fn(c)}🍀", f"v110:charm:{k}")] for k, n, c in CHARM_SHOP]
    rows.append([("🔥 ویژه‌ها", "v110:hub:0")])
    api.send_message(chat_id, ui_card("🧿 طلسم‌ها", lines,
                                      f"هر طلسم {fn(CHARM_HOURS)} ساعت دوام دارد", "🧿"),
                     inline_keyboard(rows))

def f_charm_buy(chat_id, uid, arg):
    ensure_v110()
    p = profile(uid)
    coins = int(p.get("lucky_coins") or 0)
    shop = {k: (n, c) for k, n, c in CHARM_SHOP}
    if arg not in shop:
        return
    name, cost = shop[arg]
    if coins < cost:
        api.send_message(chat_id, ui_card("🧿 طلسم‌ها",
                                          [f"❌ {fn(cost)}🍀 سکه شانس لازم داری."],
                                          f"موجودی: {fn(coins)}🍀", "🧿"))
        return
    db.execute("UPDATE profiles SET lucky_coins=MAX(0,COALESCE(lucky_coins,0)-?) WHERE user_id=?", (cost, uid))
    if arg == "energy":
        db.execute("UPDATE profiles SET energy=100 WHERE user_id=?", (uid,))
        lines = [f"⚡ «{name}» فعال شد — انرژی کاملاً پُر شد!"]
    else:
        d = _g110(uid)
        d["charm"] = {"kind": arg, "until": time.time() + CHARM_HOURS * 3600}
        _gs110(uid, d)
        lines = [f"🧿 «{name}» برای {fn(CHARM_HOURS)} ساعت فعال شد!"]
    lines += g_reward(uid, xp=30, why="طلسم")
    log_action(uid, "v110_charm", arg)
    api.send_message(chat_id, ui_card("🧿 طلسم‌ها", lines, "سکه‌هات رو خرج کن و بترکون! 🍀", "🧿"),
                     inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))


# ─────────────── 🎣 ۲۲) آکواریوم ───────────────
def f_aquarium(chat_id, uid, arg=""):
    ensure_v110()
    fishes = _gv(uid, "aqua", []) or []
    left = g_left(uid, "aqua", AQUA_DAILY)
    lines = [f"🐠 کلکسیون: {' '.join(fishes) if fishes else '— خالی'}",
             f"📊 {ui_bar(len(set(fishes)), len(AQUA_FISH))} {fn(len(set(fishes)))}/{fn(len(AQUA_FISH))} گونه",
             ui_kv("قلاب‌های امروز", fn(left))]
    rows = []
    if left > 0:
        rows.append([("🎣 بنداز قلاب", "v110:aquarium:go")])
    if len(set(fishes)) >= len(AQUA_FISH) and int(_gv(uid, "aqua_claim", 0) or 0) == 0:
        lines.append("")
        lines.append("🏆 کلکسیون کامل شد! جایزه‌ات آماده‌ست.")
        rows.append([("🎁 جایزه‌ی کلکسیون کامل", "v110:aquarium:claim")])
    rows.append([("🔥 ویژه‌ها", "v110:hub:0")])
    api.send_message(chat_id, ui_card("🎣 آکواریوم", lines,
                                      "۸ گونه جمع کن و ست رو کامل کن", "🎣"),
                     inline_keyboard(rows))

def f_aquarium_act(chat_id, uid, arg):
    ensure_v110()
    if arg == "go":
        if g_left(uid, "aqua", AQUA_DAILY) <= 0:
            api.send_message(chat_id, ui_card("🎣 آکواریوم", ["⏳ قلاب‌های امروزت تموم شد!"], None, "🎣"))
            return
        g_use(uid, "aqua")
        fishes = _gv(uid, "aqua", []) or []
        have = set(fishes)
        missing = [f for f in AQUA_FISH if f not in have]
        fish = pick(missing) if (missing and random.random() < 0.45) else pick(AQUA_FISH)
        fishes.append(fish)
        _gset(uid, "aqua", fishes)
        is_new = fish not in have
        lines = [f"🎣 قلاب انداختی… {fish} گرفتی!"]
        if is_new:
            lines.append("🆕 گونه‌ی جدید به کلکسیون اضافه شد!")
            lines += g_reward(uid, money=1200, xp=90, why="ماهی جدید")
        else:
            lines += g_reward(uid, money=250, xp=25, why="ماهی‌گیری")
        lines.append(g_map_piece(uid, 0.20))
        log_action(uid, "v110_aqua", fish)
        api.send_message(chat_id, ui_card("🎣 آکواریوم", lines,
                                          f"گونه‌ها: {fn(len(set(fishes)))}/{fn(len(AQUA_FISH))}", "🎣"),
                         inline_keyboard([[("🎣 قلاب بعدی", "v110:aquarium:go")],
                                          [("🔥 ویژه‌ها", "v110:hub:0")]]))
        return
    if arg == "claim":
        fishes = _gv(uid, "aqua", []) or []
        if len(set(fishes)) < len(AQUA_FISH):
            api.send_message(chat_id, ui_card("🎣 آکواریوم", ["🔒 کلکسیون هنوز کامل نشده!"] , None, "🎣"))
            return
        if int(_gv(uid, "aqua_claim", 0) or 0) == 1:
            api.send_message(chat_id, ui_card("🎣 آکواریوم", ["⏳ جایزه رو گرفتی!"], None, "🎣"))
            return
        _gset(uid, "aqua_claim", 1)
        lines = ["🏆 آکواریومت کامل شد! بازدیدکننده‌ها پول می‌دن!"]
        lines += g_reward(uid, money=9000, xp=400, gems=4, why="آکواریوم کامل")
        try:
            channel_news(f"🎣 «{profile(uid)['name']}» کلکسیونِ آکواریوم شهر رو کامل کرد! 🐋✨")
        except Exception:
            pass
        api.send_message(chat_id, ui_card("🎣 آکواریوم", lines, "تبریک، ماهی‌گیرِ بزرگ! 🎣", "🎣"),
                         inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))


# ─────────────── 🎭 ۲۳) تئاتر شهر ───────────────
def f_theater(chat_id, uid, arg=""):
    ensure_v110()
    left = g_left(uid, "theater", THEATER_DAILY)
    if left <= 0:
        api.send_message(chat_id, ui_card("🎭 تئاتر شهر", ["⏳ امروز ۳ اجرا داشتی؛ کف زدن‌ها تموم شد!"],
                                          "فردا دوباره روی صحنه برو", "🎭"))
        return
    g_use(uid, "theater")
    p = profile(uid)
    sk = get_skills(uid).get("comm", 0)
    score = random.randint(1, 20) + sk * 3 + (p.get("level") or 1)
    if score >= 45:
        rw, tier = (3200, 120, 1), "🌟 ایستاده تشویقت کردن!"
    elif score >= 30:
        rw, tier = (1600, 70, 0), "👏 اجرای خوبی بود!"
    elif score >= 18:
        rw, tier = (700, 35, 0), "🙂 متوسط بود"
    else:
        rw, tier = (200, 15, 0), "😬 تماشاگرها خمیازه کشیدن!"
    lines = [f"🎭 امتیازِ اجرا: {fn(score)}", "", tier]
    lines += g_reward(uid, money=rw[0], xp=rw[1], gems=rw[2], why="تئاتر")
    lines.append(g_map_piece(uid, 0.15))
    log_action(uid, "v110_theater", str(score))
    api.send_message(chat_id, ui_card("🎭 تئاتر شهر", lines,
                                      f"اجراهای باقی‌مانده: {fn(max(0, left - 1))} — ارتباطات مهارتِ اصلیه 🗣", "🎭"),
                     inline_keyboard([[("🎭 اجرای بعدی", "v110:theater")], [("🔥 ویژه‌ها", "v110:hub:0")]]))


# ─────────────── 🏅 ۲۵) تالار مشاهیر ───────────────
def _top(col, n=5, where="", order="DESC"):
    try:
        return db.fetchall(f"SELECT p.name, p.{col} AS v FROM profiles p {where} "
                           f"ORDER BY p.{col} {order} LIMIT {n}")
    except Exception:
        return []

def f_fame(chat_id, uid, arg=""):
    ensure_v110()
    lines = []
    for title, col in (("💰 ثروتمندترین‌ها", "money"),
                       ("⭐ بالاترین لول", "level"),
                       ("💎 بیشترین سکه", "gems"),
                       ("🔥 XP این هفته", "week_xp"),
                       ("♻️ بیشترین بازتولد", "rebirth"),
                       ("🏆 بیشترین اعتبار", "reputation")):
        rows = _top(col, 3)
        lines.append(f"{title}")
        if not rows:
            lines.append("   — هنوز کسی ثبت نشده")
        for i, r in enumerate(rows, 1):
            lines.append(f"   {fn(i)}. {r['name']} — {fmt_money(r['v'] or 0)}")
        lines.append("")
    api.send_message(chat_id, ui_card("🏅 تالار مشاهیر", lines,
                                      "اسمِ خودت رو اینجا ببین! 👑", "🏅"),
                     inline_keyboard([[("🏆 ماراتن ثروت", "v110:marathon")],
                                      [("👑 تاج هفته", "v110:crown")],
                                      [("🔥 ویژه‌ها", "v110:hub:0")]]))


# ─────────────── 🧨 ۲۶) بمب ساعتی ───────────────
def f_bomb(chat_id, uid, arg=""):
    ensure_v110()
    st = G("bomb", None) or {}
    holder = int(st.get("holder", 0) or 0)
    until = float(st.get("until", 0) or 0)
    now = time.time()
    lines = []
    kb = []
    if holder == 0 or until <= now:
        st = {"holder": uid, "until": now + BOMB_FUSE, "passes": 0}
        GS("bomb", st)
        lines.append("🧨 بمب دستِ توئه! زودتر پاسش بده!")
        lines.append(f"⏳ {fn(BOMB_FUSE // 60)} دقیقه وقت داری.")
        lines += g_reward(uid, money=150, xp=30, why="شروع دور بمب")
        kb.append([("💨 پاس بده به نفر بعدی", "v110:bomb:pass")])
    elif holder == uid:
        lines.append("🧨 بمب هنوز دستِ توئه!")
        lines.append(f"⏳ {ui_mmss(until - now)} تا انفجار")
        kb.append([("💨 پاس بده!", "v110:bomb:pass")])
    else:
        hp = profile(holder)
        nm = (hp or {}).get("name") or "یک شهروند"
        lines.append(f"🧨 بمب دستِ «{nm}» است.")
        lines.append(f"⏳ {ui_mmss(until - now)} تا انفجار")
        if until <= now:
            lines.append("")
            lines.append("💥 وقتش رسیده — می‌تونی بمب رو برداری و دورِ جدید رو شروع کنی!")
            kb.append([("🧨 برداشتن بمب (+جایزه)", "v110:bomb:take")])
    kb.append([("🔥 ویژه‌ها", "v110:hub:0")])
    api.send_message(chat_id, ui_card("🧨 بمب ساعتی", lines,
                                      "هرکس بمب رو نگه داره، می‌بازه! فقط پاس بده 😄", "🧨"),
                     inline_keyboard(kb))

def f_bomb_act(chat_id, uid, arg):
    ensure_v110()
    st = G("bomb", None) or {}
    holder = int(st.get("holder", 0) or 0)
    until = float(st.get("until", 0) or 0)
    ...
    if arg == "pass":
        if holder != uid:
            if holder == 0 or until <= now:
                st = {"holder": uid, "until": now + BOMB_FUSE, "passes": 0}
                GS("bomb", st)
                api.send_message(chat_id, ui_card("🧨 بمب ساعتی",
                                                  ["🎯 بمب دستِ کسی نبود — تو برداشتی!",
                                                   f"⏳ {fn(BOMB_FUSE // 60)} دقیقه وقت داری که پاسش بدی."],
                                                  "زودتر پاس بده! 💨", "🧨"),
                                 inline_keyboard([[("💨 پاس بده", "v110:bomb:pass")],
                                                  [("🔥 ویژه‌ها", "v110:hub:0")]]))
            else:
                api.send_message(chat_id, ui_card("🧨 بمب ساعتی", ["🙂 بمب دستِ تو نیست!"], None, "🧨"))
            return
        if until <= now:
            change_money(uid, -600, "v110", "انفجار بمب")
            st = {"holder": 0, "until": 0, "passes": 0}
            GS("bomb", st)
            api.send_message(chat_id, ui_card("🧨 بمب ساعتی",
                                              ["💥 بوممم! بمب دستت منفجر شد!",
                                               "💸 ۶۰۰💰 جریمه شدی."],
                                              "دفعه‌ی بعد زودتر پاس بده 😄", "🧨"),
                             inline_keyboard([[("🧨 دور جدید", "v110:bomb")],
                                              [("🔥 ویژه‌ها", "v110:hub:0")]]))
            return
        try:
            rows = db.fetchall("SELECT user_id FROM profiles WHERE user_id!=? ORDER BY RANDOM() LIMIT 1", (uid,))
        except Exception:
            rows = []
        if not rows:
            api.send_message(chat_id, ui_card("🧨 بمب ساعتی", ["❌ کسی برای پاس‌دادن پیدا نشد!"], None, "🧨"))
            return
        nxt = int(rows[0]["user_id"])
        st = {"holder": nxt, "until": until, "passes": int(st.get("passes", 0) or 0) + 1}
        GS("bomb", st)
        lines = [f"💨 بمب رو پاس دادی! رفت دستِ یک شهروندِ دیگر."]
        lines += g_reward(uid, money=200, xp=40, why="پاس دادن بمب")
        lines.append(g_map_piece(uid, 0.20))
        try:
            api.send_message(nxt, ui_card("🧨 هشدار!",
                                          ["🧨 یکی بمب رو انداخت دستِ تو!",
                                           f"⏳ {ui_mmss(until - now)} تا انفجار"],
                                          "زودتر پاسش بده! (🔥 ویژه‌ها → بمب ساعتی)", "🧨"))
        except Exception:
            pass
        log_action(uid, "v110_bomb", "pass")
        api.send_message(chat_id, ui_card("🧨 بمب ساعتی", lines, "فعلاً در امانی 😎", "🧨"),
                         inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))
        return
    if arg == "take":
        if holder == uid or until > now:
            return
        st = {"holder": uid, "until": now + BOMB_FUSE, "passes": 0}
        GS("bomb", st)
        lines = ["🎯 بمب رو برداشتی و دورِ جدید شروع شد!"]
        lines += g_reward(uid, money=250, xp=50, why="شروع دور بمب")
        api.send_message(chat_id, ui_card("🧨 بمب ساعتی", lines,
                                          f"{fn(BOMB_FUSE // 60)} دقیقه وقت داری — پاسش بده!", "🧨"),
                         inline_keyboard([[("💨 پاس بده", "v110:bomb:pass")],
                                          [("🔥 ویژه‌ها", "v110:hub:0")]]))


# ─────────────── 🌙 ۲۷) بازار شبانه ───────────────
def f_night(chat_id, uid, arg=""):
    ensure_v110()
    hr = datetime.now().hour
    if not (NIGHT_FROM <= hr < NIGHT_TO):
        api.send_message(chat_id, ui_card("🌙 بازار شبانه",
                                          [f"🌞 بازارِ شبانه بسته است.",
                                           f"🕐 ساعتِ کاری: {fn(NIGHT_FROM)} تا {fn(NIGHT_TO)} شب"],
                                          "شب‌ها برگرد! 🌙", "🌙"),
                         inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))
        return
    stock = G("night", None) or {}
    if stock.get("day") != today():
        picks = random.sample(BLACK_MARKET_ITEMS, min(3, len(BLACK_MARKET_ITEMS)))
        stock = {"day": today(),
                 "items": [{"id": p[0], "name": p[1], "price": int(p[2] * 0.7), "desc": p[3]} for p in picks]}
        GS("night", stock)
    bought = _gv(uid, "night_buy", []) or []
    lines = ["🌙 ویترینِ امشب (۳۰٪ تخفیف):", ""]
    rows = []
    for i, it in enumerate(stock.get("items", [])):
        mark = "✅" if it["id"] in bought else "🛒"
        lines.append(f"{mark} {it['name']} — {fmt_money(it['price'])}💰")
        lines.append(f"   ▫️ {it['desc']}")
        if it["id"] not in bought:
            rows.append([(f"🛒 خرید {it['name']}", f"v110:night:{i}")])
    rows.append([("🔥 ویژه‌ها", "v110:hub:0")])
    api.send_message(chat_id, ui_card("🌙 بازار شبانه", lines,
                                      "فقط تا نیمه‌شب — هر قلم یک‌بار در شب", "🌙"),
                     inline_keyboard(rows))

def f_night_buy(chat_id, uid, arg):
    ensure_v110()
    hr = datetime.now().hour
    if not (NIGHT_FROM <= hr < NIGHT_TO):
        f_night(chat_id, uid)      # پیامِ «بازار بسته است» را نشان بده (به‌جای سکوت)
        return
    stock = G("night", None) or {}
    ...
    bought = _gv(uid, "night_buy", []) or []
    if it["id"] in bought:
        api.send_message(chat_id, ui_card("🌙 بازار شبانه", ["⏳ این قلم رو امشب خریدی!"], None, "🌙"))
        return
    p = profile(uid)
    if (p.get("money") or 0) < it["price"]:
        api.send_message(chat_id, ui_card("🌙 بازار شبانه",
                                          [f"❌ {fmt_money(it['price'])}💰 لازم داری."],
                                          f"موجودی: {fmt_money(p['money'] or 0)}💰", "🌙"))
        return
    change_money(uid, -it["price"], "v110", f"بازار شبانه: {it['name']}")
    bought.append(it["id"])
    _gset(uid, "night_buy", bought)
    lines = [f"🌙 «{it['name']}» خریدی!"]
    if it["id"] == "energy_drink":
        db.execute("UPDATE profiles SET energy=MIN(100,energy+35) WHERE user_id=?", (uid,))
        lines.append("⚡ +۳۵ انرژی")
    elif it["id"] == "med_kit":
        db.execute("UPDATE profiles SET health=MIN(100,health+40) WHERE user_id=?", (uid,))
        lines.append("❤️ +۴۰ سلامت")
    elif it["id"] == "party_pack":
        db.execute("UPDATE profiles SET happiness=MIN(100,happiness+30) WHERE user_id=?", (uid,))
        lines.append("😊 +۳۰ شادی")
    elif it["id"] == "gem_shard":
        add_gems(uid, 1)
        lines.append("💎 +۱ سکه طلا")
    elif it["id"] == "xp_scroll":
        lines += g_reward(uid, xp=120, why="طومار تجربه")
    elif it["id"] == "rep_paper":
        db.execute("UPDATE profiles SET reputation=MIN(100,reputation+10) WHERE user_id=?", (uid,))
        lines.append("🏆 +۱۰ اعتبار")
    elif it["id"] == "lucky_coin":
        db.execute("UPDATE profiles SET lucky_coins=COALESCE(lucky_coins,0)+3 WHERE user_id=?", (uid,))
        lines.append("🍀 +۳ سکه شانس")
    elif it["id"] == "freeze_pack":
        db.execute("UPDATE profiles SET streak_freezes=MIN(3,COALESCE(streak_freezes,0)+1) WHERE user_id=?", (uid,))
        lines.append("❄️ +۱ فریز استریک")
    elif it["id"] == "mystery_card":
        lines += g_reward(uid, money=900, xp=80, why="کارت مرموز")
    else:
        from datetime import timedelta as _td
        db.execute("UPDATE profiles SET focus_until=? WHERE user_id=?",
                   ((datetime.now() + _td(hours=2)).isoformat(timespec="seconds"), uid))
        lines.append("🕶 دو ساعت XP ×۲")
    lines.append(g_map_piece(uid, 0.20))
    log_action(uid, "v110_night", it["id"])
    api.send_message(chat_id, ui_card("🌙 بازار شبانه", lines, "فردا شب ویترین عوض می‌شه 🌙", "🌙"),
                     inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))


# ─────────────── 🎲 ۲۸) تاس سرنوشت ───────────────
def f_dice(chat_id, uid, arg=""):
    ensure_v110()
    left = g_left(uid, "dice", DICE_DAILY)
    if left <= 0:
        api.send_message(chat_id, ui_card("🎲 تاس سرنوشت", ["⏳ پرتاب‌های امروزت تموم شد!"],
                                          f"فردا {fn(DICE_DAILY)} پرتابِ تازه", "🎲"))
        return
    g_use(uid, "dice")
    ...
    s = d1 + d2
    if s == 12:
        rw, tier = (5000, 200, 2), "🎊 دوازده! افسانه‌ای!"
    elif s >= 10:
        rw, tier = (1800, 90, 0), "🔥 عالی!"
    elif s >= 7:
        rw, tier = (800, 45, 0), "🙂 خوبه"
    elif s >= 4:
        rw, tier = (300, 20, 0), "😐 معمولی"
    else:
        rw, tier = (80, 8, 0), "😬 بدشانسی!"
    faces = {1: "⚀", 2: "⚁", 3: "⚂", 4: "⚃", 5: "⚄", 6: "⚅"}
    lines = [f"🎲 {faces[d1]} {faces[d2]}  →  {fn(s)}", "", tier]
    lines += g_reward(uid, money=rw[0], xp=rw[1], gems=rw[2], why="تاس سرنوشت")
    lines.append(g_map_piece(uid, 0.15))
    api.send_message(chat_id, ui_card("🎲 تاس سرنوشت", lines,
                                      f"پرتاب‌های باقی‌مانده: {fn(max(0, left - 1))}", "🎲"),
                     inline_keyboard([[("🎲 پرتاب بعدی", "v110:dice")], [("🔥 ویژه‌ها", "v110:hub:0")]]))


# ─────────────── 🧭 ۲۹) قطب‌نمای شانس ───────────────
def f_compass(chat_id, uid, arg=""):
    ensure_v110()
    d = _g110(uid)
    if int(d.get("compass_mult", 0) or 0) > 0:
        api.send_message(chat_id, ui_card("🧭 قطب‌نمای شانس",
                                          ["🧭 قطب‌نما از قبل فعاله!",
                                           "▫️ پاداشِ بعدیِت در بخشِ «ویژه‌ها» دوبرابر می‌شود."],
                                          "یکی از ویژه‌ها رو اجرا کن تا مصرف بشه", "🧭"))
        return
    if not g_once_today(uid, "compass"):
        api.send_message(chat_id, ui_card("🧭 قطب‌نمای شانس",
                                          ["⌛ امروز قطب‌نما رو چرخوندی!"],
                                          "فردا دوباره", "🧭"))
        return
    d["compass_mult"] = 1
    _gs110(uid, d)
    dirs = ["شمالِ ثروت 💰", "شرقِ دانش 📚", "جنوبِ شادی 😊", "غربِ قدرت ⚔️"]
    lines = [f"🧭 قطب‌نما چرخید و روی «{pick(dirs)}» ایستاد!",
             "",
             "▫️ پاداشِ بعدی‌ات در بخشِ «🔥 ویژه‌های ۱.۱.۰» ×۲ می‌شود!"]
    lines += g_reward(uid, xp=30, why="چرخش قطب‌نما")
    log_action(uid, "v110_compass", "")
    api.send_message(chat_id, ui_card("🧭 قطب‌نمای شانس", lines,
                                      "هوشمند استفاده‌اش کن — روی بزرگ‌ترین جایزه!", "🧭"),
                     inline_keyboard([[("🔥 برو سراغ ویژه‌ها", "v110:hub:0")]]))


# ─────────────── 👑 ۳۰) تاجِ هفته ───────────────
def f_crown(chat_id, uid, arg=""):
    ensure_v110()
    try:
        rows = db.fetchall("SELECT user_id, name, COALESCE(week_xp,0) AS wx FROM profiles "
                           "ORDER BY week_xp DESC LIMIT 5")
    except Exception:
        rows = []
    my_rank = None
    my_xp = int(profile(uid).get("week_xp") or 0)
    lines = [f"👑 رقابتِ هفته ({week_key()})", ""]
    for i, r in enumerate(rows, 1):
        mark = "⬅️ تو" if int(r["user_id"]) == uid else ""
        if int(r["user_id"]) == uid:
            my_rank = i
        lines.append(f"{['🥇','🥈','🥉','4️⃣','5️⃣'][i-1]} {r['name']} — {fmt_money(r['wx'] or 0)}⭐ {mark}")
    lines.append("")
    lines.append(ui_kv("XP این هفته‌ی تو", f"{fmt_money(my_xp)}⭐"))
    kb = []
    if my_rank and my_rank <= 3:
        rw = {1: (20000, 400, 8), 2: (12000, 250, 4), 3: (7000, 150, 2)}[my_rank]
        if _gv(uid, "crown_week") != week_key():
            lines.append(f"🎁 جایزه‌ی رتبه‌ی {fn(my_rank)}: {fmt_money(rw[0])}💰 | {fn(rw[1])}⭐ | {fn(rw[2])}💎")
            kb.append([("🎁 دریافت جایزه‌ی تاج", "v110:crown:get")])
        else:
            lines.append("✅ جایزه‌ی این هفته رو گرفتی!")
    else:
        lines.append("🌱 این هفته جزو ۳ نفرِ اول نیستی — XP جمع کن!")
    kb.append([("🏅 تالار مشاهیر", "v110:fame")])
    kb.append([("🔥 ویژه‌ها", "v110:hub:0")])
    api.send_message(chat_id, ui_card("👑 تاجِ هفته", lines,
                                      "هر دوشنبه رقابت از نو شروع می‌شود", "👑"),
                     inline_keyboard(kb))

def f_crown_get(chat_id, uid, arg=""):
    ensure_v110()
    if _gv(uid, "crown_week") == week_key():
        api.send_message(chat_id, ui_card("👑 تاجِ هفته",
                                          ["⏳ جایزه‌ی این هفته رو قبلاً گرفتی!"],
                                          "هفته‌ی بعد دوباره بدرخش 👑", "👑"))
        return
    try:
        rows = db.fetchall("SELECT user_id FROM profiles ORDER BY week_xp DESC LIMIT 3")
    except Exception:
        rows = []
    my_rank = None
    for i, r in enumerate(rows, 1):
        if int(r["user_id"]) == uid:
            ...
            break
    if not my_rank:
        api.send_message(chat_id, ui_card("👑 تاجِ هفته", ["🔒 جزو ۳ نفرِ اول نیستی!"], None, "👑"))
        return
    rw = {1: (20000, 400, 8), 2: (12000, 250, 4), 3: (7000, 150, 2)}[my_rank]
    _gset(uid, "crown_week", week_key())
    lines = [f"👑 تاجِ رتبه‌ی {fn(my_rank)} بر سرت گذاشته شد!"]
    lines += g_reward(uid, money=rw[0], xp=rw[1], gems=rw[2], why="تاج هفته")
    lines.append(g_map_piece(uid, 0.30))
    try:
        channel_news(f"👑 «{profile(uid)['name']}» تاجِ XP این هفته رو گرفت! رتبه‌ی {fn(my_rank)} 🏆")
    except Exception:
        pass
    log_action(uid, "v110_crown", str(my_rank))
    api.send_message(chat_id, ui_card("👑 تاجِ هفته", lines, "هفته‌ی بعد دوباره 👑", "👑"),
                     inline_keyboard([[("🔥 ویژه‌ها", "v110:hub:0")]]))
...
def panel_god(chat_id, uid, page=0):
    """🔥 مرکز ویژه‌های ۱.۱.۰ — ۳۰ فیچر در ۴ صفحه"""
    ensure_v110()
    if not guard_character(chat_id, uid):
        return
    try:
        page = max(0, min(3, int(page)))
    except Exception:
        page = 0
    start = page * V110_PER_PAGE
    chunk = V110_CATALOG[start:start + V110_PER_PAGE]
    p = profile(uid)
    d = _g110(uid)
    coins = int(p.get("lucky_coins") or 0)
    pieces = int(_gv(uid, "map_pieces", 0) or 0)
    mult = "×۲ فعال! 🧭" if int(d.get("compass_mult", 0) or 0) > 0 else "—"
    dna = next((n for k, n, c, desc in DNA_GENES if k == (d.get("dna") or "")), "—")
    head = [ui_kv("🍀 سکه شانس", fn(coins)),
            ui_kv("🗺️ تکه نقشه", f"{fn(min(pieces, G110_MAP_PIECES))}/{fn(G110_MAP_PIECES)}"),
            ui_kv("🧭 ضریب بعدی", mult),
            ui_kv("🧬 ژن", dna)]
    body = [""]
    for key, name, desc in chunk:
        body.append(f"{name}｜{desc}")
    nav = [(("▶️ " if i == page else "") + f"صفحه {fn(i + 1)}", f"v110:hub:{i}") for i in range(4)]
    rows = [[(name, f"v110:{key}")] for key, name, _d in chunk]
    rows.append(nav)
    api.send_message(chat_id,
                     ui_card(f"🔥 ویژه‌های ۱.۱.۰ — صفحه {fn(page + 1)}/۴",
                             head + body,
                             f"{fn(len(V110_CATALOG))} قابلیتِ تازه | روی هر کدام بزن تا باز شود ✨", "🔥"),
                     inline_keyboard(rows))


# ─────────────── 🧭 دیسپچرِ کال‌بک‌های ۱.۱.۰ ───────────────
def handle_v110_callback(chat_id, uid, data, cb_id):
    ensure_v110()
    cmd = data[5:]                       # بعد از "v110:"
    if cmd.startswith("hub"):
        try:
            pg = int(cmd.split(":", 1)[1])
        except Exception:
            pg = 0
        api.answer_callback(cb_id)
        panel_god(chat_id, uid, pg)
        return
    parts = cmd.split(":", 1)
    key = parts[0]
    arg = parts[1] if len(parts) > 1 else ""
    fn = V110_ROUTES.get(key)
    if not fn:
        api.answer_callback(cb_id, "⚠️")
        return
    try:
        ...
        log.exception(f"v110 {key}: {_e}")
        try:
            api.send_message(chat_id, ui_card("⚠️ خطا",
                                              ["یه خطای موقت پیش اومد؛ دوباره امتحان کن 🙏"], None, "⚠️"))
        except Exception:
            pass


# ثبتِ مسیرها
def f_marathon_route(chat_id, uid, arg=""):
    """مسیریابِ ماراتن: بدون آرگومان = نمایش پنل، با get = دریافت جایزه"""
    if arg == "get":
        f_marathon_get(chat_id, uid)
    else:
        f_marathon(chat_id, uid)


V110_ROUTES.update({
    "marathon": f_marathon_route,
    "slot":     f_slot,
    "escape":   f_escape_pick,
    "darts":    f_darts,
    "brain":    f_brain_pick,
    "oracle":   f_oracle,
    "wish":     f_wish_pick,
    ...
    "map":      f_map_open,
    "gig":      f_gig_act,
    "letter":   f_letter_pick,
    "life_ins": f_life_ins_act,
    "loot":     f_loot,
    "time_travel": f_time_travel,
    "predict":  f_predict_act,
    "build":    f_build_act,
    "locker":   f_locker_put,
    "circus":   f_circus_claim,
    "charm":    f_charm_buy,
    "aquarium": f_aquarium_act,
    "theater":  f_theater,
    "streak_battle": f_streak_battle,
    "fame":     f_fame,
    "bomb":     f_bomb_act,
    "night":    f_night_buy,
    "dice":     f_dice,
...
# نگاشتِ «نمایشِ پنل» — وقتی آرگومان خالی است این صدا زده می‌شوند
V110_PANELS = {
    "escape":   f_escape,
    "brain":    f_brain,
    "wish":     f_wish,
...
def _v110_dispatch(chat_id, uid, key, arg):
    """اگر آرگومان خالی بود پنل را نشان بده، وگرنه اکشن را اجرا کن"""
    if not arg and key in V110_PANELS:
        V110_PANELS[key](chat_id, uid)
        return
    fn = V110_ROUTES.get(key)
    if fn:
        fn(chat_id, uid, arg) if arg else fn(chat_id, uid)

# جایگزینیِ دیسپچر با نسخه‌ی آگاه‌به‌پنل
def _handle_v110(chat_id, uid, data, cb_id):
    ensure_v110()
    cmd = data[5:]
    if cmd.startswith("hub"):
        try:
    ...
    try:
        api.answer_callback(cb_id)
        _v110_dispatch(chat_id, uid, key, arg)
    except Exception as _e:
        log.exception(f"v110 {key}: {_e}")
        try:
            api.send_message(chat_id, ui_card("⚠️ خطا", ["یه خطای موقت پیش اومد؛ دوباره امتحان کن 🙏"], None, "⚠️"))
        except Exception:
            pass

handle_v110_callback = _handle_v110
...
║   بازی متنی شبیه‌ساز زندگی — نسخه ۱.۱.۰    ║
║        ⚡ ساخته‌ی تیم XR ⚡                ║
╚══════════════════════════════════════════╝
"""
...
def main():
    ...
    try:
        ensure_v109()
        ensure_v110()   # 🆕 v1.1.0: جدول مشترک + ستون JSONِ ویژه‌ها
