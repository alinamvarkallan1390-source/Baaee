#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Life Simulator — Rubika edition.
Reply Keyboard only; no inline/glass UI.
"""
import os,time,random,sqlite3,threading,logging,shutil,requests
from datetime import datetime

TOKEN=os.getenv("RUBIKA_BOT_TOKEN","TEST_TOKEN_REPLACE_ME")
ADMIN_IDS={1975639269,558945434}|{int(x) for x in os.getenv("RUBIKA_ADMIN_IDS","").replace(" ","").split(",") if x.isdigit()}
DB=os.getenv("LIFE_SIM_DB",os.path.join(os.path.dirname(os.path.abspath(__file__)),"life_simulator_rubika.db"))
LOCK=threading.RLock(); logging.basicConfig(level=logging.INFO,format="%(asctime)s | %(levelname)s | %(message)s"); log=logging.getLogger("life")
WORK_CD,REST_CD,TRAIN_CD,MARKET_CD=45,60,30,1200
JOBS={"بیکار":(500000,0),"کارگر":(800000,15),"راننده":(1200000,18),"فروشنده":(1500000,20),"برنامه‌نویس":(2500000,25),"مهندس":(3500000,25),"پزشک":(5000000,30),"کارآفرین":(7000000,30)}
...
class Rubika:
 ...
 def call(self,m,p=None,timeout=45):
  ...
  except Exception as e:log.warning("API %s: %s",m,e);return {}
 def updates(self,offset=None):
  p={"limit":100};
  if offset:p["offset_id"]=offset
  return self.call("getUpdates",p)
 def send(self,chat,text,k=None):
  p={"chat_id":str(chat),"text":str(text)[:4096]}
  if k:p["chat_keypad_type"]="New";p["chat_keypad"]={"rows":[{"buttons":[{"id":b,"type":"Simple","button_text":b} for b in row]} for row in k],"resize_keyboard":True}
...
def rows(sql,args=()):
 with LOCK,db() as c:return [dict(x) for x in c.execute(sql,args).fetchall()]
def ensure(uid,name="بازیکن",username=""):
 uid=str(uid)
 with LOCK,db() as c:
  if not c.execute("SELECT 1 FROM users WHERE uid=?",(uid,)).fetchone():c.execute("INSERT INTO users(uid,name,username,created) VALUES(?,?,?,?)",(uid,name[:80],username[:80],time.time()))
...
def gainxp(uid,n):
 p=get(uid);x=p["xp"]+int(n);lvl=p["level"]
 while x>=lvl*100:x-=lvl*100;lvl+=1;addmoney(uid,lvl*100000,"level",str(lvl))
 upd(uid,xp=x,level=lvl)

def K(*r):return [list(x) for x in r]
MAIN=K(("👤 پروفایل","💰 اقتصاد"),("💼 کار و شغل","🏢 شرکت"),("📈 بازار بورس","⚔️ نبرد"),("🎁 روزانه","🎒 دارایی‌ها"),("👥 دعوت دوستان","🏆 رتبه‌بندی"),("🎲 رویداد","ℹ️ راهنما"))
ECON=K(("💵 موجودی","🏦 بانک"),("💸 انتقال","🎁 جوایز"),("🔙 بازگشت",))
JOB=K(("🔎 شغل‌ها","💼 کار کردن"),("📚 آموزش","🛌 استراحت"),("🔙 بازگشت",))
COMP=K(("🏗 ساخت شرکت","📊 شرکت من"),("👷 استخدام","📈 عرضه سهام"),("💼 درآمد شرکت","🔙 بازگشت"))
MKT=K(("📊 قیمت‌ها","🛒 خرید سهم"),("💰 فروش سهم","📦 سهام من"),("🔙 بازگشت",))
BAT=K(("⚔️ نبرد تمرینی","👹 باس روزانه"),("🏆 رکورد نبرد","🔙 بازگشت",))
ADM=K(("👥 آمار","📢 پیام همگانی"),("💰 جایزه همگانی","💾 بکاپ"),("🚫 مدیریت مسدودی","🔙 خروج"))
...
def work(uid):
 ...
 salary,_=JOBS.get(p["job"],JOBS["بیکار"]);inc=salary+random.randint(0,max(1,salary//5));upd(uid,money=p["money"]+inc,energy=p["energy"]-20,last_work=time.time(),happiness=max(0,p["happiness"]-1));gainxp(uid,15);logit(uid,"work",inc,p["job"]);return f"💼 شیفت تمام شد!\n💰 +{fmt(inc)} تومان\n⚡ -۲۰ انرژی\n⭐ +۱۵ XP"
def rest(uid):
 p=get(uid)
...
def train(uid):
 p=get(uid)
 if time.time()-p["last_train"]<TRAIN_CD:return "⏳ آموزش بعدی آماده نیست."
 if p["energy"]<15:return "⚡ انرژی کافی نداری."
 st=random.choice(["intelligence","strength","charisma"]);upd(uid,**{st:p[st]+2,"energy":p["energy"]-15,"last_train":time.time()});gainxp(uid,10);return "📚 آموزش موفق!\n📈 یک مهارت +۲\n⭐ XP +۱۰"
def daily(uid):
...
def company(uid):
 ...
 if not c:return "🏢 شرکتی نداری.\n\nساخت شرکت: ۱ میلیارد تومان"
 return f"🏢 {c['name']}\nمحصول: {c['product']}\nارزش: {fmt(c['value'])}\n👷 کارکنان: {c['employees']}\n📈 سهام: {c['shares']}\n💹 قیمت سهم: {fmt(c['share_price']) if c['share_price'] else 'ثبت نشده'}\nوضعیت: {'بورسی' if c['listed'] else 'خصوصی'}"
def make_company(uid):
 p=get(uid);cost=1000000000
 if p["money"]<cost:return f"❌ سرمایه کافی نیست.\nلازم: {fmt(cost)} تومان"
 if one("SELECT owner FROM companies WHERE owner=?",(str(uid),)):return "❌ قبلاً شرکت داری."
 name=f"شرکت {p['name'][:18]}";prod=random.choice(["فناوری","غذا","خودرو","انرژی","فین‌تک","پوشاک"])
...
def list_company(uid):
 ...
 if c["value"]<4000000000:return f"🔒 حداقل ارزش: ۴ میلیارد\nفعلی: {fmt(c['value'])}"
 price=max(100000,c["value"]//1000)
 with LOCK,db() as x:x.execute("UPDATE companies SET listed=1,shares=1000,share_price=? WHERE owner=?",(price,str(uid)));x.commit();return f"📈 شرکت وارد بورس شد!\n۱۰۰۰ سهم\nقیمت اولیه: {fmt(price)} تومان"
def company_income(uid):
 c=one("SELECT * FROM companies WHERE owner=?",(str(uid),))
 if not c:return "❌ شرکت نداری."
 n=c["employees"]*random.randint(1000000,3000000);addmoney(uid,n,"company_income",c["name"]);return f"💼 درآمد شرکت\n💰 +{fmt(n)} تومان"

def market_tick():
 with LOCK,db() as c:
  rs=c.execute("SELECT * FROM market").fetchall()
  if not rs or time.time()-rs[0]["updated"]<MARKET_CD:return
  for r in rs:
  ...
  c.commit()
def market():
 market_tick();rs=rows("SELECT * FROM market ORDER BY symbol");return "📈 بازار مجازی\n━━━━━━━━━━\n"+"\n".join(f"{r['symbol']} | {r['name']} | {fmt(r['price'])} | {'📈' if r['price']>=r['prev'] else '📉'}" for r in rs)
def buy(uid,sym,n):
 try:n=int(n)
 except:return "❌ تعداد نامعتبر."
 if n<=0 or n>100000:return "❌ تعداد نامعتبر."
 market_tick();r=one("SELECT * FROM market WHERE symbol=?",(sym.upper(),));p=get(uid)
 if not r:return "❌ نماد پیدا نشد."
 cost=int(r["price"]*n*1.02)
 if p["money"]<cost:return f"❌ موجودی کافی نیست؛ نیاز: {fmt(cost)}"
 ...
 upd(uid,money=p["money"]-cost);logit(uid,"buy",-cost,sym.upper());return f"🛒 خرید {sym.upper()} × {n}\n💸 -{fmt(cost)} تومان"
def sell(uid,sym,n):
 try:n=int(n)
 except:return "❌ تعداد نامعتبر."
 sym=sym.upper();r=one("SELECT * FROM market WHERE symbol=?",(sym,));h=one("SELECT * FROM stocks WHERE uid=? AND symbol=?",(str(uid),sym))
 if not r or not h or n<=0 or h["amount"]<n:return "❌ سهام کافی نیست."
 ...
 with LOCK,db() as c:
  ...
  c.commit()
 addmoney(uid,gain,"sell",sym);return f"💰 فروش {sym} × {n}\n💰 +{fmt(gain)} تومان"
def my_stocks(uid):
 rs=rows("SELECT * FROM stocks WHERE uid=? AND amount>0",(str(uid),));return "📦 سهام من\n\n"+("\n".join(f"{r['symbol']}: {r['amount']} سهم | میانگین {fmt(r['avg_price'])}" for r in rs) if rs else "سهامی نداری.")
...
def battle(uid,boss=False):
 ...
 gainxp(uid,5);return "⚔️ شکست خوردی، ولی ⭐ +۵ XP گرفتی."
def referrals(uid):return f"👥 دعوت دوستان\n\nکد دعوت: REF-{uid}\n🎁 پاداش دعوت موفق: ۱٬۰۰۰٬۰۰۰ تومان\n💎 پاداش کاربر جدید: ۱ جم\n\nکاربر جدید: /start REF-{uid}"
def apply_ref(uid,code):
 if not code.startswith("REF-"):return
 ref=code[4:];p=get(uid);parent=get(ref)
 if not parent or ref==str(uid) or p["ref_by"]:return
...
def event(uid):
 p=get(uid);kind=random.choice([0,1,2,3])
 if kind==0:n=random.randint(100000,1000000);addmoney(uid,n,"event");return f"🎲 رویداد مالی!\n💰 +{fmt(n)} تومان"
 if kind==1:upd(uid,happiness=min(100,p["happiness"]+10));return "🎲 رویداد مثبت! 😊 شادی +۱۰"
 if kind==2:upd(uid,intelligence=p["intelligence"]+3);return "🎲 رویداد آموزشی! 🧠 هوش +۳"
 return "📰 خبر بازار: امروز نوسان قیمت‌ها بیشتر از حالت عادی است."
def leaderboard():
 rs=rows("SELECT name,money,level FROM users WHERE banned=0 ORDER BY money DESC LIMIT 10");return "🏆 ثروتمندترین‌ها\n\n"+"\n".join(f"{i}. {r['name']} — {fmt(r['money'])} — Lv.{r['level']}" for i,r in enumerate(rs,1))
def admin_stats():
...
def helptext():return "ℹ️ Life Simulator روبیکا\n\nبا ۱۰۰ میلیون تومان شروع می‌کنی. شغل، بانک، شرکت، بورس، دعوت دوستان، رویداد و نبرد داری.\n\n/start [کد دعوت]\n/profile\n/jobs\n/job نام شغل\n/pay شناسه مبلغ\n/deposit مبلغ\n/withdraw مبلغ\n/buy SYMBOL تعداد\n/sell SYMBOL تعداد\n/hire\n/admin\n/broadcast متن\n/grant مبلغ\n/ban شناسه\n/unban شناسه"
def backup():
...
def process(uid,text,name="بازیکن",username=""):
 ...
 if text=="💰 اقتصاد":return "💰 اقتصاد — انتخاب کن:",ECON
 ...
 if text=="🎁 جوایز":return f"🎁 جوایز\n💎 جم: {p['gems']}\n🔥 استریک: {p['streak']}",ECON
 ...
 if text=="⚔️ نبرد":return "⚔️ نبرد\n\nنبرد تمرینی و باس روزانه برای XP و جایزه.",BAT
 ...
 if text in ("ℹ️ راهنما","/help"):return helptext(),MAIN
 if text=="🔙 بازگشت":return "🔙 منوی اصلی",MAIN
 if text=="/admin" and is_admin(uid):return "🛠 پنل مدیریت",ADM
 if is_admin(uid) and text=="👥 آمار":return admin_stats(),ADM
 if is_admin(uid) and text=="💾 بکاپ":return "💾 بکاپ ساخته شد: "+os.path.basename(backup()),ADM
 if is_admin(uid) and text=="📢 پیام همگانی":return "📢 /broadcast متن",ADM
 if is_admin(uid) and text=="💰 جایزه همگانی":return "💰 /grant مبلغ",ADM
 if is_admin(uid) and text=="🚫 مدیریت مسدودی":return "🚫 /ban شناسه\n/unban شناسه",ADM
 if is_admin(uid) and text=="🔙 خروج":return "خروج از پنل.",MAIN
 a=text.split()
 if a and a[0]=="/job":
  j=" ".join(a[1:]);
 ...
 if a and a[0]=="/pay" and len(a)>=3:
  ...
  if n<=0 or t==str(uid) or not get(t):return "❌ انتقال نامعتبر.",ECON
 ...
 if a and a[0]=="/deposit" and len(a)==2:
  ...
  upd(uid,money=p["money"]-n,bank=p["bank"]+n);return f"🏦 +{fmt(n)} تومان به بانک واریز شد.",ECON
 if a and a[0]=="/withdraw" and len(a)==2:
  try:n=int(a[1])
  except:return "❌ مبلغ نامعتبر.",ECON
  if n<=0 or p["bank"]<n:return "❌ موجودی بانک کافی نیست.",ECON
  upd(uid,money=p["money"]+n,bank=p["bank"]-n);return f"💵 {fmt(n)} تومان برداشت شد.",ECON
 if a and a[0]=="/buy" and len(a)>=3:return buy(uid,a[1],a[2]),MKT
 if a and a[0]=="/sell" and len(a)>=3:return sell(uid,a[1],a[2]),MKT
 if a and a[0]=="/hire":return hire(uid),COMP
 if is_admin(uid) and a and a[0]=="/grant" and len(a)==2:
  try:n=int(a[1])
  except:return "❌ مبلغ نامعتبر.",ADM
  ids=[r["uid"] for r in rows("SELECT uid FROM users WHERE banned=0")]
  for x in ids:addmoney(x,n,"admin_grant")
  return f"💰 {fmt(n)} به {len(ids)} کاربر داده شد.",ADM
 if is_admin(uid) and a and a[0]=="/broadcast" and len(a)>1:
  msg=" ".join(a[1:]);ids=[r["uid"] for r in rows("SELECT uid FROM users WHERE banned=0")];ok=0
  for x in ids:
   if api.send(x,msg,MAIN):ok+=1
   time.sleep(.03)
  return f"📢 ارسال: {ok}/{len(ids)}",ADM
 if is_admin(uid) and a and a[0] in ("/ban","/unban") and len(a)==2:
  if not get(a[1]):return "❌ کاربر پیدا نشد.",ADM
  upd(a[1],banned=1 if a[0]=="/ban" else 0);return ("🚫 مسدود شد." if a[0]=="/ban" else "✅ رفع مسدودی شد."),ADM
...
def run():
 init_db();offset=None;log.info("Life Simulator Rubika started")
 while True:
  try:
   r=api.updates(offset);us=r.get("updates",[]) if isinstance(r,dict) else []
   if isinstance(r,dict) and r.get("next_offset_id") is not None:offset=str(r["next_offset_id"])
   for u in us:
    try:
